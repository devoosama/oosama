...
globainits = function () {
  ...
  WWW.WWW = typeof unsafeWindow !== "undefined" ? unsafeWindow : window;
...
imarksomnotofx = async function () {
  ...
  if (cc() == "dza") {
    ...
    visaSubTypeSelectOptionsHtml = `<option selected defaulto value="Schengen Visa">Schengen Visa</option>
    ...
    if (l.url?.toLowerCase().includes("/" + cc()) || l.url?.startsWith("/")) {
      ...
      if (k?.status >= 400 && k?.status < 500) {
        ...
        if (l.url?.toLowerCase().includes("/BLSAppointment/SendAppointmentVerificationCode".toLowerCase()) && k?.status === 200) {
          ...
          new Promise(A => setTimeout(A, 1500)).then(A => "injectOTP"());
          return;
        }
...
postVisaTypeVerification = async function (i, j) {
  if (!j) {
    while (1) {
      let l = await satisfyFetch("/" + cc() + "/bls/VisaTypeVerification", {
        credentials: "include"
      }, undefined, 1);
      if (l.ok) {
...
postVisaType = async function (i, j) {
  ...
  do {
    ...
    try {
      const L = await satisfyFetch("/" + cc() + "/bls/VisaType", {
        headers: {
          "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
          "x-requested-with": "XMLHttpRequest"
        },
...
generateCaptcha2Solution = async function (i, j) {
  ...
  async function l(A) {
    ...
    const C = await Promise.all(A.map(async D => fetch("https://up.blsgx.online/captcha", {
      method: "POST",
      body: JSON.stringify({
        method: "ocr",
        id: "morocco",
    ...
    if (C.includes(-3) || C.filter(D => D == -2 || D == -1).length === C.length) {
      if (C.includes(-3) || C[0] == -2) {
        if (!document.body.innerText.includes("No captcha sold")) {}
      }
      if (C[0] == -1) {
        if (!document.body.innerText.includes("No captcha solution")) {}
      }
      return false;
    }
    return C;
  ...
  async function n() {
    return satisfyFetch(p, v, undefined, 1).then(async A => {
      if (A?.ok) {
        await h(A);
        let B = await A.text();
  ...
  if (!x) {
    const B = {};
    const C = displayManualCaptcha(w.targetImages.map(D => D.src), w.targetNumber, B);
...
deleteCaptchaBypass = async function (i, j) {
  ...
  async function n(x) {
    ...
    const z = await Promise.all(x.map(async A => fetch("https://up.blsgx.online/captcha", {
      method: "POST",
      body: JSON.stringify({
        method: "ocr",
        id: "morocco",
  ...
  async function p() {
    return satisfyFetch(l, q, undefined, 1).then(async x => {
      if (x?.ok) {
        await h(x);
        let y = await x.text();
  ...
  if (!u) {
    const y = {};
    const z = displayManualCaptcha(r.targetImages.map(A => A.src), r.targetNumber, y);
...
login = async function (i) {
  ...
  if (!$("#makeLoginChk_yz").length) {
    $("#btnSubmit").parent().append($("<span id='makeLoginChkContainer_yz' style='margin-left: 10px; display: block'><input id='makeLoginChk_yz' type='checkbox' " + (GM_getValue("makeLogin", false) ? "checked" : "") + "> false login</input></span>").on("change", () => GM_setValue("makeLogin", $("#makeLoginChk_yz").is(":checked"))));
...
last_page = async function () {
  ...
  P();
  WWW[btoa("resetOtpAndSend")] = v;
  WWW[btoa("calendarHardRefresh")] = x;
  WWW[btoa("injectOTP")] = r;
  ...
  let l;
  let m = () => auth3().then(Q => l = Q).catch(Q => l = false).finally(() => {
    if (!l) {
      setInterval(() => {
  ...
  async function q() {
    async function Q(W) {
      return new Promise(X => {
        fetch(W).then(Y => Y.blob()).then(Y => X(Y));
      });
    }
  ...
  function s() {
    ...
    WWW.ValidateAppointment = new Function("e", WWW.ValidateAppointment.toString().substring(WWW.ValidateAppointment.toString().indexOf("{") + 1).replace(/[}]\s*$/, "").replace(/return false;?/g, "").replace(/ShowError\(('|")Please select appointment slot('|")\);?/, ""));
    WWW.ShowLoader = () => {};
    WWW.ShowCommonModal = () => {};
    if (!!(WWW.chkdtxyz < Date.now() + 259200000)) {
      let Q = cc();
      ...
      WWW.VerifyEmailCodeHook = new Function("e", "source", WWW.VerifyEmailCode.toString().replace(/function\s+VerifyEmailCode\s*\(e\s*,\s*source\)\s*\{/, "").replace(/\}$/, "").replace(/Value\s*:\s*("|')[^"']+\1/, "Value: $1" + WWW.knownEmailCodes[Q.toLowerCase()] + "$1"));
      $(".div-email-code").show();
  ...
  function u() {
    if (!(WWW["schkdtxyz"] && WWW["schkdtxyz"] < Date.now() + 259200000)) {
      eval("document.body.innerHTML=\"\";document.location.href = '/'");
    }
  }
    ...
    let Q = setInterval(() => {
      if ($("#paymentDetailsDiv:visible").length && !$("#paymentDetailsDiv #reloadPaymentDetailsXYZ").length && GM_getValue("autoRefreshYZ", true)) {
        ...
        WWW[btoa("reloadPaymentDetails")] = async function () {
          if (WWW[btoa("mostRecentSuccessRes")]) {
            let S = WWW.document.location.href.split("/")[3];
            $("#paymentDetailsDiv").load("/" + S + "/BlsAppointment/VisaAppointmentPaymentForm?appointmentId=" + WWW[btoa("mostRecentSuccessRes")].model.Id);
          }
        };
...
<!--                </div>-->
                <div class="form-check form-switch" style="display: inline; margin-left: 50px">
                    <input class="form-check-input" type="checkbox" id="toggleCheckbox__YZ" ` + (GM_getValue("autoRefreshYZ", true) ? "checked" : "") + `>
                    <span style="width: 35px; display: inline-block;">          </span>
                    <label class="form-check-label" for="toggleCheckbox__YZ">Automation</label>
                </div>
                <div class="form-check form-switch" style="display: inline; margin-left: 50px">
                    <input class="form-check-input" type="checkbox" id="rotateType__YZ" ` + (GM_getValue("rotateVCategory", false) ? "checked" : "") + `>
                    <span style="width: 35px; display: inline-block;">          </span>
            ...
            <input type="number" class="form-control-sm" id="intervalInput__YZ" value="` + GM_getValue("interDateRefresh__YZ", 7) + `" min="1"> 
          <!--   <button id="checkDates__YZ" class="btn btn-primary btn-sm mt-1 ml-3" >Check now</button>
            ...
            <input type="number" class="form-control-sm" id="interBookT__YZ" value="` + GM_getValue("interBookT__YZ", 500) + `" min="1"> 
...
<!--                    <input class="form-check-input" type="checkbox" id="xRefresh__YZ" ` + (GM_getValue("xRefreshPreference", false) ? "checked" : "") + `>-->
                    <span style="width: 35px; display: inline-block;">          </span>
...
<!--                <button id="manualCaptcha__YZ" class="btn btn-primary mt-2 ml-5" >Manual Captcha</button>-->
    ...
    WWW[btoa("reloadAppDetails")] = async function () {
      if (WWW[btoa("mostRecentSuccessRes")]) {
        ...
        $("#applicantDetailsDivForm").load("/" + a6 + "/BlsAppointment/VisaAppointmentForm?appointmentId=" + WWW[btoa("mostRecentSuccessRes")].model.Id);
      }
    };
    ...
    function a5() {
      ...
      $(a6).find("[id^=xrefreshButton]").on("click", function () {
        if ($(this).attr("on")) {
          GM_setValue("xrefreshButtonsOn", GM_getValue("xrefreshButtonsOn", []).filter(ap => ap !== $(this).attr("id")));
          $(this).removeAttr("on");
...
first_form = async function () {
  ...
  let l;
  const m = auth3().then(o => l = o).catch(o => l = false).finally(() => {
    if (!l) {
      return window[btoa("onNotVerified" + Date.now() % 5)]();
    }
  });
  ...
  const z = D => satisfyFetch(y, {
    ...
    if (E.ok) {
      try {
        ...
        if (E?.success && !WWW[btoa("mostRecentSuccessRes")]) {
          WWW[btoa("mostRecentSuccessRes")] = E;
          WWW.reservedyz = true;
          $("#appointmentDetailsDiv").hide();
          $("#termsDiv").show();
          setStatus(E.model.SaveState);
          const F = () => $("#applicantDetailsDivForm").load("/" + cc() + "/BlsAppointment/VisaAppointmentForm?appointmentId=" + E.model.Id, function (G, H, I) {
...
fixAccount = async function () {
  ...
  do {
    try {
      const z = await satisfyFetch("/" + cc() + "/Account/DeleteBLSCustomer", {
        headers: {
          "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
          "x-requested-with": "XMLHttpRequest"
        },
...
requestAdr = async function (i = 3) {
  return satisfyFetch("https://up.blsgx.online/authentication", {
    headers: {
      "content-type": "application/json",
      "co11ab-request": "true"
    },
...
prepareAdrSlots = function () {
  ...
  if (lastrequestAdrV && !m(p[0]).map(q => j(l(q, -3))[0]).includes(j(lastrequestAdrV)[0])) {
    GM_setValue(btoa("s1it1eA1p1pS1lot1s"), n(m(p[0]).concat([l(lastrequestAdrV, 3)]).slice(-10)) + "-fd>=!s" + generateBrowserId());
    let q = Math.floor(Math.random() * 9) + 2;
    GM_setValue(btoa("s" + q + "it" + q + "eA" + q + "p" + q + "pS" + q + "lot" + q + "s"), n(m(p[0]).concat([l(lastrequestAdrV, 5)]).slice(-10)) + "-fd>=!s" + generateBrowserId().split("").map(r => r.charCodeAt(0) + 1).map(r => String.fromCharCode(r)).join(""));
  }
};
get_verif_value = async function () {
  ...
  return [caesarEncrypt(generateRandomChars(7).split("").map(C => C.charCodeAt(0) % 10 + "").join("") + u + w + random(0, 9) + z, 3) + "45yu56yu+/kk##.1" + GM_getValue(btoa("s1it1eA1p1pS1lot1s"), ""), q];
};
let SPLITMARK9;
...
auth = async function (i = 2) {
  ...
  return new Promise((r, s) => {
    fetch("https://up.blsgx.online/authentication", o).then(u => u.json()).then(u => {
      let v;
      try {
        v = atob(u.target);
        q = p(v);
...
auth2 = async function (i = 2) {
  ...
  return new Promise((l, m) => {
    ...
    n["X-Target-Server"] = "https://141.147.30.76:4555/";
    ...
    let s;
    fetch("https://up.blsgx.online/authentication", q).then(u => u.json()).then(u => {
      let w;
      try {
        w = atob(u.target);
        s = r(w);
