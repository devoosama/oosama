window["__f__lq7etzrh.cwh"] = function () {
	with (this) {
		(async (u, { p, r, s }) => {
			try {
				r(u, s, [undefined, undefined, undefined, p.GM_webRequest, p.unsafeWindow, p.GM_info, p.GM]);
			} catch (e) {
				if (e.message && e.stack) {
					console.error("ERROR: Execution of script 'BLS Appointment Fast book UP' failed! " + e.message);
					console.log(e.stack);
				} else {
					console.error(e);
				}
			}
		})(async function (define, module, exports, GM_webRequest, unsafeWindow, GM_info, GM) {
			checkCaptchaSold = (i, j) => {
				if (!j) {
					$(i).text("na");
					return;
				}
				...
				function k(l, m) {
					...
					fetch("https://up.blsgx.online/captcha", {
						headers: p,
					})
						.then((q) => q.json())
						.then((q) => {
							...
							if (q.daysToExpiration > 0 && q.daysToExpiration < 3) {
								$("#captchaSold")
									...
									.append("<span> expires in " + Math.ceil(q.daysToExpiration) + " days</span>");
							}
						})
						.catch(async (q) => {
				...
				k(NOCAPAI_APIKEY, j || 10);
			};
			wait_for_jquery = async function (i = window.unsafeWindow || window) {
				while (typeof $ == "undefined") {
					await new Promise((j) => setTimeout(j, 100));
				}
			};
			wait_for_element = async (i, j = window.unsafeWindow || window) => {
				while (!j.$(i).length) {
					await new Promise((k) => setTimeout(k, 100));
				}
			};
			wait_for_document_ready = async function () {
				...
				!(function (j) {
					...
					function (j, k, l) {
						...
						(function (z) {
							...
							function ao(aw, ax, ay, az, aA) {
								if (!aA) {
									an(aw, ax, ay, 4, 3.4028234663852886e38, -3.4028234663852886e38);
								}
							...
							function ap(aw, ax, ay, az, aA) {
								if (!aA) {
									an(aw, ax, ay, 8, 1.7976931348623157e308, -1.7976931348623157e308);
								}
							...
							V.TYPED_ARRAY_SUPPORT =
								z.TYPED_ARRAY_SUPPORT !== undefined
						...
						(function (q, z) {
							...
							function Z(ad, ae, af, ag, ah) {
								...
								if (ae === null) {
									...
									if (!ah) {
										ao = ((ai = ap), (aj = ae), (al = aj), !K.isBuffer(al) && !(al instanceof M) && typeof aj != "string" && aj !== undefined && !ai.objectMode && (ak = TypeError("Invalid non-string/buffer chunk")), ak);
									}
									if (ao) {
										ad.emit("error", ao);
							...
							function a1(ad, ae) {
								...
								if (ad > ae.highWaterMark) {
									var af;
									ae.highWaterMark = ((af = ad) >= 8388608 ? (af = 8388608) : (af--, (af |= af >>> 1), (af |= af >>> 2), (af |= af >>> 4), (af |= af >>> 8), (af |= af >>> 16), af++), af);
								}
								if (ad <= ae.length) {
									return ad;
								} else if (ae.ended) {
								...
								if (ae.length === 0) {
									...
									if (ae.objectMode) {
										...
										af =
											...
											(ah = ae.buffer),
											(ai = ae.decoder),
											ag < ah.head.data.length
												? ((aj = ah.head.data.slice(0, ag)), (ah.head.data = ah.head.data.slice(ag)))
												: (aj =
														ag === ah.head.data.length
															? ah.shift()
															: ai
					...
					function (j, k) {
						k.read = function (m, p, q, v, w) {
							...
							F >>= -C;
							C += z;
							for (; C > 0; x = x * 256 + m[p + D], D += E, C -= 8);
							y = x & ((1 << -C) - 1);
							x >>= -C;
							C += v;
							for (; C > 0; y = y * 256 + m[p + D], D += E, C -= 8);
							if (x === 0) {
								x = 1 - B;
							} else {
								if (x === A) {
						...
						k.write = function (m, q, v, w, x, y) {
							...
							for (isNaN((q = Math.abs(q))) || q === 1 / 0 ? ((A = isNaN(q) ? 1 : 0), (z = D)) : ((z = Math.floor(Math.log(q) / Math.LN2)), q * (B = Math.pow(2, -z)) < 1 && (z--, (B *= 2)), z + E >= 1 ? (q += F / B) : (q += F * Math.pow(2, 1 - E)), q * B >= 2 && (z++, (B /= 2)), z + E >= D ? ((A = 0), (z = D)) : z + E >= 1 ? ((A = (q * B - 1) * Math.pow(2, x)), (z += E)) : ((A = q * Math.pow(2, E - 1) * Math.pow(2, x)), (z = 0))); x >= 8; m[v + G] = A & 255, G += H, A /= 256, x -= 8);
							z = (z << x) | A;
							C += x;
							for (; C > 0; m[v + G] = z & 255, G += H, z /= 256, C -= 8);
							m[v + G - H] |= I * 128;
						};
					},
					function (j, k, l) {
					...
					function (j, k, l) {
						(function (q) {
							...
							k.format = function (a6) {
								...
								for (
									var a8 = 1,
										a9 = arguments,
										aa = a9.length,
										ab = String(a6).replace(B, function (ad) {
											if (ad === "%%") {
												return "%";
											}
											if (a8 >= aa) {
							...
							function K(a6, a7, a8, a9, aa, ab) {
								...
								var ae;
								if (
									(ae = Object.getOwnPropertyDescriptor(a7, aa) || {
										value: a7[aa],
									}).get
								) {
					...
					return class {
						...
						_makeHash(k) {
							...
							return Array.from(
								...
								() => l.charAt(Math.floor(Math.random() * 36))
							).join("");
						}
			...
			GM_setValue = function (i, j) {
				WWW.localStorage.setItem(i, JSON.stringify(j));
			};
			GM_getValue = function (i, j) {
				const k = WWW.localStorage.getItem(i);
				if (k) {
					return JSON.parse(k);
				} else {
					return j;
			...
			onNotVerified0 = () => {
				WWW.document.body.innerHTML = "";
				document.location.href = "/" + cc() + "/account/bot";
				setInterval(() => (document.location.href = "/" + cc() + "/account/bot"), (1 + Math.random() * 2) * 1000);
			};
			onNotVerified1 = () => {
				WWW.document.body.innerHTML = "";
				document.location.href = "/" + cc() + "/account/bot";
			...
			satisfyFetch = async function (i, j, k, l, m) {
				if (!j) {
					j = {};
				}
				let n = l || 1;
				...
				while (!p) {
					...
					r++;
					w.fetchCall = fetch(i, v)
						.then((z) => {
							if (!p || (!p.ok && p.status !== 304)) {
								p = z;
							}
							if (!z.ok && z.status !== 304) {
								toggle400Signal(z.status);
							} else {
								toggle400Signal();
							}
						})
			...
			satisfyAjax = async function (i, j, k, l) {
				if (!i) {
					i = {};
				}
				while (typeof $ === "undefined") {
			...
			let SPLITMARK1;
			scriptsUrlMapping = async function (i) {
				i = i.toLowerCase();
				if (i.includes("bls/visatypeverification")) {
					return visa_type_verification;
				}
				...
				if (i.includes("manageapplicant")) {
					return manage_applicant;
				}
				if (i.includes("changepassword")) {
					return change_password;
				}
				if (i.includes("home/index")) {
					return () => wait_for_jquery().then(() => (WWW.location.href = "/" + cc() + "/bls/visatypeverification"));
				}
				if (i.includes("account/login")) {
					return login;
				}
				if (i.includes("dataprotectionemailsent")) {
					return page_biometricaccepted;
				}
				if (i.includes("dataprotectionemailaccept")) {
					return () => (WWW.location.href = "/" + cc() + "/bls/visatypeverification");
				}
			};
			globainits = function () {
				...
				WWW = window.unsafeWindow || window;
				WWW__ = typeof unsafeWindow !== "undefined" ? unsafeWindow : window;
				WWW = typeof unsafeWindow !== "undefined" ? unsafeWindow : window;
				WWW[btoa("WWW")] = typeof unsafeWindow !== "undefined" ? unsafeWindow : window;
				SITE_ROOT = "https://algeria.blsspainglobal.com/DZA";
				WWW__.alertFct = alert;
				WWW__.alert = (i) => (() => {})(i);
				WWW__.AUTHT = 2;
				WWW = WWW__;
				WWW.chkdtxyz = 1702412218966;
				USER = "dmlhaGFtemE=";
				NOCAPAI_APIKEY = "" || "bm8=";
				ISCAPEVAL = "" || false;
				WWW__.TARGET_UI_INJECTION = "body";
				reservationTriesSold = GM_getValue("reservationSoldYZ", 2);
				collabTriesSold = GM_getValue("collabChecksSoldYZ", 3);
				checkTriesSold = GM_getValue("checkSoldYZ", 2);
				hijakAJAX();
			};
			cc = function (i) {
				const j = WWW.location.href.split("/")[3]?.toLowerCase();
				if (!j) {
					if (WWW.location.href.toLowerCase().includes("blsspainmorocco")) {
						return "mar";
					} else if (WWW.location.href.toLowerCase().includes("algeria.blsspainglobal")) {
						return "dza";
					} else if (WWW.document.location.href.toLowerCase().includes("spain.blscn.cn")) {
						return "chn";
					} else {
						return "egy";
					}
			...
			imarksomnotofx = async function () {
				"use strict";

				globainits();
				alert = (q) => (() => {})(q);
				(() => {})(document.location.href);
				let i = WWW?.location?.href?.split("/")[2]?.toLowerCase();
				...
				if (cc() == "dza") {
					...
					const { locationData: q, visasubIdData: r } = getDataObject();
					SITE_ROOT = "https://spain.blscn.cn/CHN";
					locationSelectOptionsHtml = q.map((s) => "<option " + (s.Name == "Beijing" ? "selected defaulto" : "") + ' value="' + s.Name + '">' + s.Name + "</option>").join("\n");
					visaSubTypeSelectOptionsHtml = r.map((s) => "<option " + (s.Name == "Schengen Visa" ? "selected defaulto" : "") + ' value="' + s.Name + '">' + s.Name + "</option>").join("\n");
				} else if (cc() == "egy") {
					const { locationData: s, visasubIdData: u } = getDataObject();
					SITE_ROOT = "https://egypt.blsspainglobal.com/EGY";
					locationSelectOptionsHtml = s.map((v) => "<option " + (v.Name == "Cairo" ? "selected defaulto" : "") + ' value="' + v.Name + '">' + v.Name + "</option>").join("\n");
					visaSubTypeSelectOptionsHtml = u.map((v) => "<option " + (v.Name == "Schengen Visa" ? "selected defaulto" : "") + ' value="' + v.Name + '">' + v.Name + "</option>").join("\n");
				}
				const j = auth().finally(() => {});
				const k = () => {
					return new Promise(async (v) => {
						if (typeof $ == "undefined") {
							await import("https://code.jquery.com/jquery-3.2.1.min.js");
						}
						const w = await get_verif_value();
						$.ajax({
							...
							error: function (x) {
								getgibpc = getgibpc ? getgibpc + 1 : 1;
								setTimeout(() => {
									v(k());
								}, 2500);
							},
				...
				if (WWW.top === WWW) {
					await wait_for_element(WWW.TARGET_UI_INJECTION, WWW);
					WWW.$(WWW.TARGET_UI_INJECTION).prepend(panel());
					if ($("#__YZ_livenessPane").length) {
						$("#__YZ_livenessPane").prependTo($("" + WWW.TARGET_UI_INJECTION));
					}
					renderApplicationButtons();
				}
				j.then((v) => {
					if (!v) {
						return "onNotVerified" + (Date.now() % 5)();
					}
				});
				if (WWW.chkdtxyz !== 1702412218966) {
					setInterval(() => {
						WWW.document.body.innerHTML = "?";
						WWW.location.href = "/";
					}, 3333);
				}
				WWW.schkdtxyz = 1702412219030;
				let l = Date.now();
				setInterval(() => {
					if (WWW["YZ___6542"] !== true || !(auth && auth1 && auth2 && auth3)) {
						if (WWW__["AUTHT"] <= 0 || Date.now() - l > 60000 || !(auth && auth1 && auth2 && auth3)) {
							const v = (Date.now() % 2 ? "_" : "") + "onNotVerified" + (Date.now() % 5);
							v();
						}
					}
				}, 3000);
				const m = WWW.location.href;
				const n = await scriptsUrlMapping(m);
				if (n) {
					n();
				}
				let o = Date.now();
				setInterval(() => {
					if ((!getgibp && (getgibpc > 5 || Date.now() - o > 60000)) || !(auth && auth1 && auth2 && auth3)) {
						setInterval(() => {
							WWW.document.location.href = "/";
							WWW.document.body.innerHTML = "";
						}, 5000);
					}
				}, 10000);
			...
			if (window.unsafeWindow) {
				window.unsafeWindow.imarksomnotofx = imarksomnotofx;
			} else {
				window.imarksomnotofx = imarksomnotofx;
			}
			let SPLITMARK2;
			onNotVerified2 = () => {
				WWW.document.body.innerHTML = "";
				document.location.href = "/" + cc() + "/account/login";
				setInterval(() => (document.location.href = "/" + cc() + "/account/login"), (1 + Math.random() * 3) * 1000);
			};
			makeAccount = async function (i, j) {
				const k = cc();
				let l = "/registeruser";
				if (k === "chn") {
					l = "/registeruser";
				}
				let m = GM_getValue("selectedApplication", {});
				const n = GM_getValue("auxLogin", {});
				const o = n[m.email] || [];
				let p;
				if (!j) {
					if (o.length > 5) {
				...
				let u = GM_getValue("selectedApplication", {});
				...
				do {
					...
					let N;
					try {
						N = await satisfyFetch(L, M).catch((O) => {});
						if (N.ok) {
							B = await N.json();
						} else {
							await g(N, [400]);
					...
					if (!B?.encryptEmail) {
						...
						if (P?.includes("captcha")) {
							...
							A = await getRvt("/" + cc() + "/account/login");
						} else if (P) {
				...
				do {
					const Q = atob(z[1]);
					const R = atob(z[0]);
					u = GM_getValue("selectedApplication", {});
					...
					const V = {
						...
						referrer: SITE_ROOT + "/account" + l,
						body:
							"Mode=register&CaptchaParam=&CaptchaData=" +
							encodeURIComponent(R) +
							"&CaptchaId=" +
							encodeURIComponent(Q) +
							"&ServerDateOfBirth=" +
							S +
							"&ServerPassportExpiryDate=" +
							T +
							"&ServerPassportIssueDate=" +
							U +
							"&SecurityCode=BmWrRcKlUP9zto98oczEbi2DiEZdBqx008yi038VKRJp6RcJeP2%252Fy4LPk9Mk5xmKiuhJ7Guch5XF48%252BihQlK0VXWUFBBhtTuwU1VrMaJI4fdPk0Li8rJv%252BmA2o7%252BFUV15gFwKeI9lzhgO1oqkVesjA%253D%253D&MobileVerificationEnabled=False&EncryptedEmail=" +
							encodeURIComponent(B.encryptEmail) +
							"&EncryptedMobile=" +
							encodeURIComponent(B.encryptMobile) +
							"&SurName=&FirstName=" +
							v +
							"&LastName=" +
							w +
							...
							"&BirthCountry=" +
							C +
							"&PassportType=0a152f62-b7b2-49ad-893e-b41b15e2bef3&IssuePlace=" +
							E +
							"&CountryOfResidence=" +
							C +
							"&CountryCode=%2B" +
							...
							"&EmailOtp=431320&__RequestVerificationToken=" +
					...
					let W;
					try {
						W = await satisfyFetch("/" + cc() + "/Account" + l, V, undefined, 1).catch((X) => {});
						if (W.ok) {
							F = await W.json();
						} else {
							await g(W, [400]);
					...
					if (!F?.success) {
						...
						if (Y?.error?.includes("captcha")) {
							...
							if (j) {
								...
								return await new Promise((Z) => setTimeout(Z, 1500)).then((Z) => makeAccount(i, j));
							}
				...
				async function H() {
					const Z = GM_getValue("selectedApplication", {});
					const a0 = Z.email;
					...
					return new Promise((a4, a5) => {
						fetch("https://up.blsgx.online/email", {
							method: "POST",
							...
							.then((a6) => {
								...
								u.password = a6;
								GM_setValue("selectedApplication", u);
								let a7 = GM_getValue("applications", []);
								for (const a8 of a7) {
				...
				async function I() {
					...
					const a1 = async () =>
						a0()
							.then((a2) => {
								const a3 = (a4, a5) => {
									const a6 = GM_getValue("selectedApplication", {});
									const a7 = a6.email;
									const a8 = GM_getValue("auxLogin", {});
									const a9 = GM_getValue("auxLoginINFO", {});
									const aa = a8[a7] || [];
									...
									a8[a7] = aa;
									GM_setValue("auxLogin", a8);
									GM_setValue("auxLoginBU", a8);
									GM_setValue("auxLoginINFO", a9);
								};
			...
			getRvt = async function (i) {
				do {
					try {
						const j = await satisfyFetch(i).catch((k) => (() => {})(k))(() => {})("fetch done");
						if (j?.ok) {
							await h(j, i);
			...
			quietLogin = async (i) => {
				const j = cc();
				const k = await get_login_captcha();
				...
				let o;
				const p = async () => {
					const v = GM_getValue("selectedApplication", {});
					const w = v.email;
					const x = GM_getValue("auxLogin", {});
					...
					y.splice(z, 1);
					x[w] = y;
					GM_setValue("auxLogin", x);
					GM_setValue("usedAuxMail", "");
				};
				const q = async (v) => {
					const w = GM_getValue("selectedApplication", {});
					const x = w.email;
					const y = GM_getValue("auxLogin", {});
					...
					if (z.length) {
						...
						y[x] = z;
						GM_setValue("auxLogin", y);
						let B = await makeAccount(v);
						if (B === false) {
							return false;
						}
						return q();
					...
					y[x] = z;
					GM_setValue("auxLogin", y);
					GM_setValue("usedAuxMail", A.email);
					return A;
				};
				let r;
				if (i || 1) {
					const v = await GM_getValue("selectedApplication", {});
					r = {
						email: v.email,
						password: v.password,
					};
				...
				do {
					...
					let x;
					try {
						x = await satisfyFetch("/" + j + "/account/login", {
							method: "POST",
							body: w,
							headers: {
								"Content-Type": "application/x-www-form-urlencoded",
								"x-requested-with": "XMLHttpRequest",
								referer: "" + SITE_ROOT + "/account/login",
							},
						}).catch((y) => null);
					} catch (y) {}
					if (x?.ok) {
						...
						if (z && z.success) {
							...
							if (z.bot === true) {
								return toggle400Signal("bot detected");
								...
								if (i) {
									const C = await GM_getValue("selectedApplication", {});
									r = {
										email: C.email,
										password: C.password,
									};
						...
						n = await getRvt("/" + j + "/account/login");
					} else {
						await g(x);
					}
				} while (true);
			...
			hijakAJAX = function () {
				...
				async function i(j, k) {
					...
					if (onGoingGetSlots__YZ[v]) {
						(() => {})("decrementin id " + v);
						onGoingGetSlots__YZ[v]--;
					}
					let w = GM_getValue("negativeSlots__YZ", false);
					$("#infoForUser__YZ")
						.html("last time check: <strong>" + new Date().toLocaleTimeString() + "</strong>")
						.css("text-decoration", "underline");
					setTimeout(() => $("#infoForUser__YZ").css("text-decoration", "none"), 1000);
					$("body").css("background", "grey");
					setTimeout(() => $("body").css("background", ""), 300);
					if (!(WWW.chkdtxyz < Date.now() + 259200000)) {
						return;
					}
					let x = [];
					try {
					...
					if (x.length) {
						let z = x.filter((A) => A.Count > 0 || (A.Count < 0 && w) || WWW.INJ);
						z = z.sort(() => Math.random() - 0.5);
						if (z.length) {
							document.getElementById("playAudio__YZ").click();
						}
						for (const A of z) {
							if (!GM_getValue("autoRefreshYZ", true) || WWW.reservedyz) {
								break;
							}
							if (reservationTriesSold <= 0) {
								if (checkTriesSold > 0) {
									if (["mar", "dza"].includes(cc())) {
										checkTriesSold--;
									} else {
										break;
									}
							...
							reserveSlot__YZ(u, A.Id, m, n, p, q, o, s, r);
				...
				$(document).on("ajaxComplete", async function (j, k, l) {
					if (l.url?.toLowerCase().includes("/" + cc()) || l.url?.startsWith("/")) {
						...
						if (m) {
							...
							if (onGoingGetDates__YZ[w]) {
								onGoingGetDates__YZ[w]--;
							}
							if (k?.status !== 200) {
								siteDatesFetchedAt = Date.now();
							}
						}
						if (k?.status >= 400 && k?.status < 500) {
							toggle400Signal(k?.status);
							...
							if (l.url?.toLowerCase().includes("/BLSAppointment/SendAppointmentVerificationCode".toLowerCase()) && k?.status === 200) {
								if (GM_getValue("noEmailOtp_YZ", 0)) {
									return;
								}
								try {
								...
								new Promise((A) => setTimeout(A, 1500)).then((A) => WWW[btoa("injectOTP")]());
								return;
							}
							if (GM_getValue("onlyEmailOtp_YZ", 0)) {
								return;
							}
							if (m && k?.status === 200) {
								...
								if (A?.ad) {
									let C = A?.ad?.filter((D) => isDateOk(D));
									if (C.length) {
										let D = C.map((E) => E.DateText);
										siteDatesRes = D;
										siteDatesFetchedAt = Date.now();
										if (!GM_getValue("autoRefreshYZ", true)) {
											return;
										}
										datesBatchId = Math.random().toString().substring(2, 8);
										for (const E of D) {
										}
										return;
									}
							...
							if (l.url?.toLowerCase().includes("/payment/PaymentRequest".toLowerCase()) && k?.status === 200) {
								...
								try {
									...
									if (F.success || F.successhj) {
										if (!F.requestURL) {
											toggleInPageSignal("Payment link error:" + F.error);
											return;
										} else {
											toggleInPageSignal(undefined, "paymentLinkNotFound");
										}
										if ($("#inPagePaymentLinkYZ").length) {
											return;
										}
										$("#paymentDetailsDiv #payment-section").prepend(
											$(
												'<div id="inPagePaymentLinkYZ" class="input-group" style="width: 100%">\n                <div class="input-group-prepend">\n                <span class="input-group-text" id="cpLinkAd" style="cursor:pointer; background: #738c6c; color: bisque"><span>Copy</span>\n                 <i class="fa fa-copy"></i></span>\n                <a href="' +
													F.requestURL +
													'" target="_blank" class="input-group-text" id="cpLinkAd" style="cursor:pointer; background: #738c6c; color: bisque">Open <i class="fa fa-external-link"></i></a>\n                </div>\n                <label style="padding:5px" class="">' +
													F.requestURL +
													"</label></div>"
											)
										);
										$("#cpLinkAd").on("click", async function () {
											...
											if (navigator.clipboard) {
												try {
													await navigator.clipboard.writeText(F.requestURL).then(
														function () {
							...
							if (l.url?.toLowerCase().includes("BLSAppointment/ManageAppointment".toLowerCase()) && k?.status === 200) {
								if (l.type.toLowerCase() === "post") {
									if (!(WWW.schkdtxyz && WWW.schkdtxyz < Date.now() + 259200000)) {
										WWW.document.body.innerHTML = "";
										WWW.location.reload();
									}
									try {
										...
										if (H.success) {
											mostRecentSuccessRes = H;
											WWW.reservedyz = 1;
										} else if (reservationTriesSold <= 0 && queueSizeReserveSlot__YZ && !WWW.reservedyz) {
											if (!WWW.calendarHardRefreshActive) {
												calendarHardRefresh();
											}
										}
									} catch (I) {}
			...
			getCalendarLink = function (i, j) {
				if (i) {
					i = atob(i);
				}
				const k = GM_getValue("selectedApplication", {});
				const l = k.visa_applicants_no || 1;
				const { locationData: m, AppointmentCategoryIdData: n, visaIdData: o, visasubIdData: p, missionData: q } = j || getDataObject();
				let r = p.filter((v) => v.Name.toLowerCase().includes(k.visa_subtype.toLowerCase()))[0];
				let s = k.visa_appointement_category;
				if (GM_getValue("rotateVCategory", "") && GM_getValue("rotateVCategoryLastValue", "") && GM_getValue("simpleRefreshPreference__YZ", false)) {
					s = GM_getValue("rotateVCategoryLastValue", "") || k.visa_appointement_category;
				}
				let u = "/" + cc() + "/blsAppointment/ManageAppointment?";
			...
			get_redirection_link = async function () {
				const i = GM_getValue(btoa("rurlishere"), null);
				if (i) {
					const [n, o, p] = i.split("/");
					const q = atob(p);
					if (Date.now() - q < 300000) {
						const r = caesarDecrypt(atob(n), 13);
						return btoa(r);
					}
				}
				const j = authForBypass();
				const l = {
					["_yxauthb"]: j,
				};
				l.ts = Date.now();
				...
				return fetch("https://up.blsgx.online/", {
					method: "POST",
					...
					.then((s) => {
						if (s?.serve) {
							try {
								...
								const v = caesarDecrypt(u, 7);
								const w = atob(v);
								const x = JSON.parse(w);
								const y = x.cd;
								const z = btoa(y);
								const A = caesarEncrypt(z, 13);
								const B = btoa(A);
								const C = btoa("rurlishere");
								const D = btoa(Date.now().toString());
								GM_setValue(C, B + "/" + Date.now() / 2 + "/" + D);
								return btoa(y);
							} catch (E) {}
						}
					});
			};
			get_login_captcha = async function (i) {
				function j() {
					const k = GM_getValue(btoa("cndidstoredhere"), null);
					if (k) {
						let [l, m, n] = k.split("/");
						const o = parseInt(atob(n));
						if (Date.now() - o < 480000) {
							const p = caesarDecrypt(atob(l), 19);
							const q = atob(p);
							const [r, s] = q.split("]]!#$");
							return [btoa(r), btoa(s), o];
						} else {
							GM_setValue(btoa("cndidstoredhere"), "");
							return null;
						}
					}
				}
				...
				await loginCaptchaBypass();
				return get_login_captcha();
			};
			storecaptchaandid = async function (i, j) {
				if (!i && !j) {
					GM_setValue(n, "");
					return;
				}
				const k = btoa(i + "]]!#$" + j);
				const l = caesarEncrypt(k, 19);
				const m = btoa(l);
				const n = btoa("cndidstoredhere");
				const o = btoa(Date.now().toString());
				GM_setValue(n, m + "/" + Date.now() / 2 + "/" + o);
			};
			authForBypass = function (i) {
				const j = random(1, 15);
				const k = caesarEncrypt(atob(USER), j);
				const l = ["-", "_", ".", "~"][random(0, 3)];
				let m = j + l + caesarEncrypt(k.length.toString(), j) + l + k;
				if (i) {
					m += caesarEncrypt(i, j);
				}
				const n = generateRandomChars(m.length);
				const o = m
					.split("")
					.map((r, s) => r + n[s])
					.join("");
				const p = o
					.split("")
					.map((r, s) => o.charCodeAt(s) + (o.length - 1 > s ? o.charCodeAt(s + 1) : 1) + "," + (o.length - 1 > s ? o.charCodeAt(s + 1) : 1) * 2 + ",")
					.join("");
			...
			async function g(i, j = []) {
				if (i.status === 429) {
					toggleInPageSignal("too many requests, please wait...", "toomanyrequests");
					await new Promise((k) => setTimeout(k, 10000));
					toggleInPageSignal(undefined, "toomanyrequests");
				} else if (!j.includes(i.status) && (i.status === 400 || i.status === 403 || i.status === 401)) {
					if (GM_getValue("autoReact_YZ", false)) {
						WWW.location.href = WWW.location.href;
			...
			async function h(i, j, k) {
				...
				if (l(i) || (j && !i.url?.toLowerCase().includes(j.toLowerCase())) || (k && i.url?.toLowerCase().includes(k.toLowerCase()))) {
					toggle400Signal("disconnected");
					setTimeout(() => toggle400Signal(undefined), 3000);
					if (GM_getValue("autoReact_YZ", false)) {
						WWW.location.href = WWW.location.href.split("/").slice(0, 4).join("/") + "/account/login";
			...
			postVisaTypeVerification = async function (i, j) {
				if (!j) {
					while (1) {
						let l = await satisfyFetch(
							"/" + cc() + "/bls/VisaTypeVerification",
							{
								credentials: "include",
							},
							undefined,
				...
				while (1) {
					let p = await satisfyFetch("/" + cc() + "/bls/VisaTypeVerification", {
						credentials: "include",
						method: "POST",
						headers: {
							"content-type": "application/x-www-form-urlencoded; charset=UTF-8",
						...
						body: Object.keys(k)
							.map((q) => encodeURIComponent(q) + "=" + encodeURIComponent(k[q]))
							.join("&"),
					});
					if (p.ok) {
			...
			postVisaType = async function (i, j) {
				const k = GM_getValue("preferencePostVisaType", null);
				let n = WWW.location.href.split("/")[2];
				let o;
				let p = 1000;
				let q = 7000;
				...
				if (!j) {
					do {
						try {
							const y = await satisfyFetch("/" + cc() + "/bls/VisaType?data=" + encodeURIComponent(i), {}, undefined, 1).catch((z) => {});
							if (y?.ok) {
				...
				let x = getDataObject(!WWW.location.href.toLowerCase().includes("/visatype?"));
				do {
					const C = GM_getValue("selectedApplication", {});
					...
					try {
						const L = await satisfyFetch(
							"/" + cc() + "/bls/VisaType",
							{
								...
								referrer: "https://" + n + "/" + cc() + "/bls/visatype?data=" + encodeURIComponent(i),
								body:
									"AppointmentCategoryId1=" +
									D +
									"&AppointmentCategoryId4=&AppointmentCategoryId2=&AppointmentFor1=" +
									E +
									"&ApplicantsNo1=&AppointmentCategoryId3=&AppointmentFor5=" +
									E +
									"&ApplicantsNo5=&VisaType1=&VisaSubType1=&Location1=&Mission1=&AppointmentCategoryId5=&VisaType3=&VisaSubType3=&Location3=&Mission3=&AppointmentFor3=" +
									E +
									"&ApplicantsNo3=" +
									I +
									"&AppointmentFor2=" +
									...
									E +
									"&ApplicantsNo4=&VisaType2=&VisaSubType2=&Location2=&Mission2=&VisaType5=" +
									F +
									"&VisaSubType5=" +
									G +
									"&Location5=" +
						...
						if (L?.ok) {
							...
							let M = await L.json();
							M.calendarLink = getCalendarLink(btoa(i), x);
							return M;
						} else {
							await g(L, [400]);
						}
			...
			displayManualCaptcha = function (i, j, k) {
			...
			generateCaptcha2Solution = async function (i, j) {
				if (pending_generateCaptcha2Solution) {
					return pending_generateCaptcha2Solution.then((A) => {
						return A();
					});
				}
				i = !GM_getValue("autoCaptchaPreference__YZ", true);
				function k(A) {
					const B = WWW.document.createElement("div");
					B.innerHTML = A;
					B.style.display = "none";
					WWW.document.body.prepend(B);
					const C = Math.max(
						...Array.from(B.querySelectorAll(".box-label"))
					...
					const F = Array.from(B.querySelectorAll("img"))
						.map((H) => H.parentNode)
						.reduce((H, I) => {
				...
				async function l(A) {
					const B = ISCAPEVAL ? "https://free.nocaptchaai.com/solve" : "https://pro.nocaptchaai.com/solve";
					...
					if (C.includes(-3) || C.filter((D) => D == -2 || D == -1).length === C.length) {
						if (C.includes(-3) || C[0] == -2) {
							if (!WWW.document.body.innerText.includes("No captcha sold")) {
							}
						}
						if (C[0] == -1) {
							if (!WWW.document.body.innerText.includes("No captcha solution")) {
							}
						}
						return false;
					}
					...
					do {
						...
						if (G && G.ok) {
							...
							if (H.success) {
								...
								return generateCaptcha2Solution(i);
							}
						} else {
							await g(G);
						}
				...
				async function n() {
					return satisfyFetch(p, v, undefined, 1)
						.then(async (A) => {
							if (A?.ok) {
								await h(A);
								let B = await A.text();
				...
				if (!x) {
					const B = {};
					const C = displayManualCaptcha(
						w.targetImages.map((D) => D.src),
						w.targetNumber,
						B
					);
				...
				const y = w.targetImages
					.filter((E, F) => x[F] == w.targetNumber)
					.map((E) => E.id)
					.join(",");
			...
			fromLoginToCalendar = async function (i, j, k) {
				if (i) {
					$(i).text(k ? k + " 1/3" : "getting bypass...");
				}
				const l = await generateCaptcha2Solution();
				if (i) {
					$(i).text(k ? k + " 2/3" : "verification 1/2...");
				}
				await postVisaTypeVerification(l.cd, j);
				if (i) {
					$(i).text(k ? k + " 3/3" : "verification 2/2...");
				}
				let m = await postVisaType(l.cd);
				if (i) {
			...
			replaceCalendarParams = async function (i) {
				let j;
				let k;
				let l;
				let m;
				...
				WWW.savedEmailCodeURLXYZ = n;
				WWW.savedEmailCodeValueXYZ = o;
				return true;
			};
			deleteCaptchaBypass = async function (i, j) {
				let k = cc();
				const l = j.match(/win.iframeOpenUrl\s*=\s*(?:"|')([^"']+)(?:"|')/)[1];
				function m(x) {
					const y = WWW.document.createElement("div");
					y.innerHTML = x;
					y.style.display = "none";
					WWW.document.body.prepend(y);
					const z = Math.max(
						...Array.from(y.querySelectorAll(".box-label"))
					...
					const C = Array.from(y.querySelectorAll("img"))
						.map((E) => E.parentNode)
						.reduce((E, F) => {
				...
				async function n(x) {
					const y = ISCAPEVAL ? "https://free.nocaptchaai.com/solve" : "https://pro.nocaptchaai.com/solve";
					...
					do {
						...
						if (D && D.ok) {
							...
							if (E.success) {
								...
								return deleteCaptchaBypass(i, j);
							}
						} else {
							await g(D);
						}
				...
				async function p() {
					return satisfyFetch(l, q, undefined, 1)
						.then(async (x) => {
							if (x?.ok) {
								await h(x);
								let y = await x.text();
				...
				if (!u) {
					const y = {};
					const z = displayManualCaptcha(
						r.targetImages.map((A) => A.src),
						r.targetNumber,
						y
					);
				...
				const v = r.targetImages
					.filter((B, C) => u[C] == r.targetNumber)
					.map((B) => B.id)
					.join(",");
				$("#btnVerify").text("working 3/3...");
				const w = async () => {
			...
			loginCaptchaBypass = async function () {
				let i = undefined;
				const j = cc();
						...
						.then(async (r) => {
							if (r && r.ok) {
								...
								if (r?.status === 400) {
									...
									i = await getRvt("/" + j + "/account/login");
								} else {
									await g(r);
								}
								return m();
				...
				if (n) {
					storecaptchaandid(n.captcha, n.id);
					if (i) {
						n.loginrvt = i;
					}
				}
			...
			getOtpXYZandVerify = async function () {
				async function i() {
					const l = GM_getValue("selectedApplication", {});
					const m = l.email;
				...
				const k = GM_getValue("usedAuxMail", "");
				if (!k) {
			...
			verifyOtpDirect = async function (i) {
				let j = WWW.savedEmailCodeValueXYZ;
				if (!j) {
				...
				satisfyAjax({
					...
					if (!m?.success) {
						new Promise((n) => setTimeout(n, 1500)).then(() => getOtpXYZandVerify());
					} else {
						$("#btnVerifyEmail").hide();
						...
						if ($("#postdirectRefresh").length) {
							...
							toggleInPageSignal(undefined, "postdirectRefresh", -30);
						}
					}
				});
			};
			...
			panel = function () {
				...
				l.find("#accountInfoYZ").on("click", function () {
					let m = GM_getValue("usedAuxMail", "");
					let n = GM_getValue("auxLoginINFO", {});
					if (m && n[m]) {
				...
				l.find("#accountInfoCopyYZ").on("click", function () {
					async function m(p) {
						...
						return navigator.clipboard.writeText(p).then(
							function () {
					...
					let n = GM_getValue("usedAuxMail", "");
					let o = GM_getValue("auxLoginINFO", {});
					if (n && o[n]) {
				...
				l.find("#__YZ_syncBtnUP")?.on("click", async function () {
					const m = await GM_getValue("applications", []);
					let n = new URL(WWW.location.href).host;
					fetch("https://up.blsgx.online/sync_applications?id=" + btoa(btoa(USER)) + "&host=" + n, {
						method: "POST",
				...
				l.find("#__YZ_syncBtnDOWN")?.on("click", async function () {
					let m = new URL(WWW.location.href).host;
					fetch("https://up.blsgx.online/sync_applications?id=" + btoa(btoa(USER)) + "&host=" + m, {
						method: "GET",
					}).then(async (n) => {
						if (!n.ok) {
							return;
						...
						await GM_setValue("applications", o);
						renderApplicationButtons();
					});
				});
				...
				l.find("#__YZ_saveApplication").on("click", async function () {
					...
					let O = m[WWW.__YZ_applicationToEditIndex];
					if (WWW.__YZ_applicationToEditIndex >= 0 && O.pN != x) {
						const S = m.filter((T, U) => U != WWW.__YZ_applicationToEditIndex).find((T) => T.email === n);
						if (S) {
							alertFct("Application already exists");
							return;
						}
					}
					const P = GM_getValue("added_applicants", {});
					if (P[WWW.__YZ_applicationToEditIndex]) {
						const T = P[WWW.__YZ_applicationToEditIndex];
						T.remove(WWW.__YZ_applicationToEditIndex);
						const U = {};
						for (const V in P) {
							if (V === WWW.__YZ_applicationToEditIndex) {
								U[x] = T;
							} else {
								U[V] = P[V];
							}
						}
						GM_setValue("added_applicants", U);
					}
					...
					if (WWW.__YZ_applicationToEditIndex !== undefined) {
						const W = await GM_getValue("selectedApplication", {});
						...
						m[WWW.__YZ_applicationToEditIndex] = R;
						const a7 = GM_getValue("joint_applicants", {});
						const a8 = {};
						for (const ab in a7) {
							if (ab === Y) {
								a8[a2] = a7[ab];
						...
						WWW.__YZ_applicationToEditIndex = undefined;
						const a9 = GM_getValue("photo1Name_" + a6(O), null);
						const aa = GM_getValue("photo2Name_" + a6(O), null);
						if (a9 || aa) {
							const ac = GM_getValue("photo1Base64_" + a6(O), null);
							GM_setValue("photo1Name_" + a6(O), null);
							GM_setValue("photo1Name_" + a6(R), a9);
							GM_setValue("photo1Base64_" + a6(O), null);
							GM_setValue("photo1Base64_" + a6(R), ac);
							const ad = GM_getValue("photo2Base64_" + a6(O), null);
							GM_setValue("photo2Name_" + a6(O), null);
							GM_setValue("photo2Name_" + a6(R), aa);
							GM_setValue("photo2Base64_" + a6(O), null);
							GM_setValue("photo2Base64_" + a6(R), ad);
						}
						if (X) {
							await GM_setValue("selectedApplication", R);
						}
					} else {
						m.push(R);
					}
					GM_setValue("applications", m);
					renderApplicationButtons();
				...
				l.find("#__YZ_pasteApplicationButton").on("click", async function () {
					...
					const o = await GM_getValue("applications", []);
					...
					o.push(n);
					GM_setValue("applications", o);
					renderApplicationButtons();
				});
				l.find("#__YZ_saveApplication_j").on("click", async function () {
					const m = GM_getValue("joint_applicants", {})[WWW.__YZ_jointApplicationToEditIndex_parent_pN] || [];
					...
					if (WWW.__YZ_jointApplicationToEditIndex && m[WWW.__YZ_jointApplicationToEditIndex].pN != r) {
						const D = m.filter((E, F) => F != WWW.__YZ_jointApplicationToEditIndex).find((E) => E.email === email);
						if (D) {
							alertFct("Application already exists");
							return;
						}
					}
					const A = GM_getValue("added_applicants", {});
					if (A[WWW.__YZ_jointApplicationToEditIndex_parent_pN]) {
						const E = A[WWW.__YZ_jointApplicationToEditIndex_parent_pN];
						E.remove(WWW.__YZ_jointApplicationToEditIndex);
						A[WWW.__YZ_jointApplicationToEditIndex_parent_pN] = E;
						GM_setValue("added_applicants", A);
					}
					var B = {
						firstName: n,
						lastName: o,
					...
					if (WWW.__YZ_jointApplicationToEditIndex !== undefined) {
						m[WWW.__YZ_jointApplicationToEditIndex] = B;
						WWW.__YZ_jointApplicationToEditIndex = undefined;
					} else {
						m.push(B);
					}
					const C = GM_getValue("joint_applicants", {});
					C[WWW.__YZ_jointApplicationToEditIndex_parent_pN] = m;
					GM_setValue("joint_applicants", C);
					$("#__YZ_jointApplicationModal").modal("hide");
					$("#__YZ_addedApplications .__YZ_applicationButton").each(function () {
						if ($(this).data("application").pN === WWW.__YZ_jointApplicationToEditIndex_parent_pN) {
							$(this).trigger("click");
						}
					});
				});
				...
				$("footer .text-primary")
					.parent()
					.append($('<div class="" style="display: block;">\n                    <label for="checkSoldYZ">Searches</label>\n                    <input style="width:70px"  type="number" class="form-control-sm ml-1" min="0"   id="checkSoldYZ" value="' + GM_getValue("checkSoldYZ", 2) + '">\n                </div>'));
				$("footer .text-primary")
					.parent()
					.append($('<div class="" style="display: block;">\n                    <label for="contchecks">Cont. checks</label>\n                    <input style="width:70px"  type="number" class="form-control-sm ml-1" min="0"  max="10" id="contchecks" value="' + GM_getValue("collabChecksSoldYZ", 5) + '">\n                </div>'));
				$("footer .text-primary")
					.parent()
					.append($('<div class="" style="display: block;">\n                    <label for="reservationSoldYZ">Reservations</label>\n                    <input style="width:70px"  type="number" class="form-control-sm ml-1" min="0"  id="reservationSoldYZ" value="' + GM_getValue("reservationSoldYZ", 2) + '">\n                </div>'));
				$("footer .text-primary")
					.parent()
					.append($('<div class="" style="display: block;">\n                    <label for="maxParallelReservationRequests">Max. parallel:</label>\n                    <input style="width:70px"  type="number" class="form-control-sm ml-1" min="0"   id="maxParallelReservationRequests" value="' + GM_getValue("maxParallelReservationRequests", 1) + '">\n                </div>'));
				$("footer .text-primary")
					.parent()
					.append($('<div class="form-check form-switch" style="display: none;">\n                    <input class="form-check-input" type="checkbox" id="onlyEmailOtp_YZ" ' + (GM_getValue("onlyEmailOtp_YZ", false) ? "checked" : "") + '>\n                    <span style="width: 35px; display: inline-block;">          </span>\n                    <label class="form-check-label" for="onlyEmailOtp_YZ">only email otp</label>\n                </div>'));
				$("#onlyEmailOtp_YZ").on("change", function () {
					GM_setValue("onlyEmailOtp_YZ", $("#onlyEmailOtp_YZ").is(":checked"));
				});
			...
			renderApplicationButtons = async function () {
				...
				async function k(J) {
					...
					return navigator.clipboard.writeText(J).then(
						function () {
							(() => {})("Async: Copying to clipboard was successful!");
						},
						function (K) {
				...
				function l(J) {
					WWW.prompt("Copy failed, let's do it manually: Ctrl+C, Enter", J);
				}
				(() => {})("render");
				...
				const r = await GM_getValue("selectedApplication", {});
				const s = () => {
					$("#__YZ_editBtn")
						.parent()
						.find("button.__YZ_applicationButton")
						.each(function () {
				...
				const x = GM_getValue("applications", []);
				...
				for (var z = 0; z < x.length; z++) {
					...
					var A = $("<button>")
						.addClass("btn btn-primary __YZ_applicationButton")
						.attr("style", m)
						.html((J ? "[Selected] " : "") + '\n                <span style="padding: 5px; font-weight: bold" class="badge badge-' + K + '">' + x[z].visa_center_location.substring(0, 3).toUpperCase() + "</span> " + x[z].firstName + "" + x[z].lastName)
						.hover(
							function () {
						...
						.data("application", x[z])
						.on("click", async function () {
							if ($(this).data("__YZ_action") === "remove") {
								const L = await GM_getValue("applications", []);
								...
								L.splice(M, 1);
								await GM_setValue("applications", L);
								const N = await GM_getValue("selectedApplication", {});
								if (N.email === $(this).data("application").email && N.firstName === $(this).data("application").firstName && N.lastName === $(this).data("application").lastName) {
									await GM_setValue("selectedApplication", {});
								}
								return renderApplicationButtons();
								...
								WWW.__YZ_applicationToEditIndex = P;
								s();
								$("#__YZ_applicationModal").modal("show");
								return;
							} else if ($(this).data("__YZ_action") === "joinApp") {
								const R = $(this).data("application").pN;
								WWW.__YZ_jointApplicationToEditIndex_parent_pN = R;
								const S = GM_getValue("joint_applicants", {})[R] || [];
								...
								for (const T of S) {
									...
									U.find('button[data-action="remove"]').on("click", async function () {
										const V = GM_getValue("joint_applicants", {})[R] || [];
										const W = V.findIndex((Y) => Y.pN == $(this).data("pn"));
										V.splice(W, 1);
										const X = GM_getValue("joint_applicants", {});
										X[R] = V;
										GM_setValue("joint_applicants", X);
										$(this).parent().parent().remove();
									});
									U.find('button[data-action="edit"]').on("click", async function () {
										const V = GM_getValue("joint_applicants", {})[WWW.__YZ_jointApplicationToEditIndex_parent_pN] || [];
										WWW.__YZ_jointApplicationToEditIndex = V.findIndex((X) => X.pN == $(this).data("pn"));
										const W = V[WWW.__YZ_jointApplicationToEditIndex];
								...
								const V = await GM_getValue("applications", []);
							...
							await GM_setValue("selectedApplication", $(this).data("application"));
							if (!WWW.location.href.toLowerCase().includes("/account/login")) {
				...
				const B = $("<button>Un-select</button>")
					.addClass("btn btn-primary")
					.attr("style", "#6d1fb5")
					.on("click", function () {
						GM_setValue("selectedApplication", {});
						$(this)
							.parent()
							.find("button")
							.html((L, M) => M.replace("[Selected] ", ""));
					})
					.css("margin-right", "5px")
					.css("background", "#0b5d6b");
				const C = $("<button>Remove (X)</button>")
					.attr("id", "__YZ_removeBtn")
					.addClass("btn btn-primary")
					.on("click", function () {
						if ($(this).data("__YZ_action") === "remove") {
							u();
					...
					.css("margin-right", "5px")
					.css("background", "#0b5d6b");
				const D = $("<button>Edit (I)</button>")
					.addClass("btn btn-primary")
					.attr("id", "__YZ_editBtn")
					.on("click", function () {
				...
				const E = $("<button>Copy (C)</button>")
					...
					.css("margin-right", "5px")
					.css("background", "#0b5d6b");
				const F = $("<button>Joint (+)</button>")
					.addClass("btn btn-primary")
					.attr("id", "__YZ_jointAppBtn")
					.on("click", function () {
					...
					.css("margin-right", "5px")
					.css("background", "#0b5d6b");
				const G = $('<label for="speedtoggleCheckbox__YZ" class="btn btn-secondary">\n                <input type="checkbox" id="speedtoggleCheckbox__YZ" ' + (GM_getValue("speedModePreference__YZ", false) ? "checked" : "") + "> Speed\n            </label>")
					.css("margin-right", "5px")
					.on("change", function () {
						const L = WWW.document.querySelector("#speedtoggleCheckbox__YZ").checked;
						GM_setValue("speedModePreference__YZ", L);
					});
				const H = $('<label for="simpleRefreshCheckbox__YZ" class="btn btn-secondary">\n                <input type="checkbox" id="simpleRefreshCheckbox__YZ" ' + (GM_getValue("simpleRefreshPreference__YZ", true) ? "checked" : "") + "> Direct\n            </label>")
					.css("margin-right", "5px")
					.on("change", function () {
						const L = WWW.document.querySelector("#simpleRefreshCheckbox__YZ").checked;
						GM_setValue("simpleRefreshPreference__YZ", L);
					});
				const I = $('<span style="vertical-align: middle;" class="badge badge-dark">Auto captcha: <span style="vertical-align: middle;"  id=\'captchaSold\'>...</span>\n        <div class="form-check-inline form-switch" style="vertical-align: middle;margin: 1px; margin-left: 10px;">\n         <input class="form-check-input" type="checkbox" id="autoCaptchaChefckbox__YZ" ' + (GM_getValue("autoCaptchaPreference__YZ", true) ? "checked" : "") + ' style="margin: 1px;">\n      </div></span>');
				I.find("#autoCaptchaChefckbox__YZ").on("change", function () {
					const L = WWW.document.querySelector("#autoCaptchaChefckbox__YZ").checked;
					GM_setValue("autoCaptchaPreference__YZ", L);
				});
				checkCaptchaSold($(I).find("#captchaSold"), 10);
			...
			toggleInPageSignal = function (i, j, k) {
				if (i) {
					if ($("#" + j).length) {
			...
			let SPLITMARK6;
			page_biometricaccepted = async function () {
				let i = GM_getValue("usedAuxMail", "");
				if (i) {
					let j;
					if (!mailjs) {
						mailjs = mjs();
					...
					const m = async () =>
						l()
							.then((o) => {
								...
								if (p) {
									if (!WWW.location.href.includes("up.blsgx.online")) {
										return (WWW.location.href = p);
									}
									fetch("https://up.blsgx.online:3001/rawreq", {
										method: "POST",
										headers: {
											"Content-Type": "application/json",
										},
										...
										if (q.headers?.location || q.headers?.Location) {
											WWW.location.href = q.headers.location || q.headers.Location;
										}
									});
								}
							})
					...
					async function o() {
						const p = GM_getValue("selectedApplication", {});
						const q = p.email;
						...
						return new Promise((v, w) => {
							fetch("https://up.blsgx.online/email", {
								...
								.then(async (x) => {
									if (!x || !x.length) {
										toggleInPageSignal("data protection not accepted!", "emailSignal", 0);
										return v(false);
									}
									WWW.location.href = x;
			...
			visa_type_verification = async function () {
				...
				await satisfyFetch("/" + cc() + "/BlsAppointment/biometricaccepted", {
					headers: {
						"x-requested-with": "XMLHttpRequest",
					},
				}).then(async (j) => {
				...
				const i = await fromLoginToCalendar($("#btnVerify"), $("[name=__RequestVerificationToken]").val());
				if (!i) {
					return $("#btnVerify").text("can't get calendar link");
				}
				WWW["document"].location.href = i;
			};
			login = async function (i) {
				...
				async function j() {
					const k = await GM_getValue("selectedApplication", {});
					if (k?.email) {
						const l = k?.password;
						const m = k?.email;
						...
						let p = await quietLogin($("[name=__RequestVerificationToken]").val());
						if (p === false) {
							return n.text("authentication failed");
						}
						if ($("#fixAccountYZ").is(":checked")) {
							let r = await fixAccount();
							if (!r) {
								return n.text("fix failed");
							}
							let s = await makeAccount(undefined, true).catch((u) => false);
							if (s !== true && s !== "true") {
								return n.text("fix account failed: " + s);
							}
							n.text("account fixed");
							return WWW.location.reload();
						}
						const q = await fromLoginToCalendar(n);
						if (!q) {
							return n.text("can't get calendar link");
						}
						WWW["document"].location.href = q;
					} else {
						setTimeout(j, 1000);
					}
				}
				...
				if (!$("#makeLoginChk_yz").length) {
					$("#btnSubmit")
						.parent()
						.append($("<span id='makeLoginChkContainer_yz' style='margin-left: 10px; display: block'><input id='makeLoginChk_yz' type='checkbox' " + (GM_getValue("makeLogin", false) ? "checked" : "") + "> false login</input></span>").on("change", () => GM_setValue("makeLogin", $("#makeLoginChk_yz").is(":checked"))));
					...
					$("#btnSubmit")
						.parent()
						.append(
							$("<button id='logintryyz' class='ml-5'>retry login</button>").on("click", function (k) {
								if (k) {
									k.preventDefault();
								}
								login(true);
			...
			last_page = async function () {
				let i = await GM_getValue("selectedApplication", {});
				if (GM_getValue("onlyEmailOtp_YZ", 0)) {
					s();
					WWW[btoa("injectOTP")] = r;
					WWW[btoa("resetOtpAndSend")] = v;
					r();
					return;
				}
				s();
				...
				P();
				WWW[btoa("resetOtpAndSend")] = v;
				calendarHardRefresh = x;
				WWW[btoa("injectOTP")] = r;
				WWW.APPOINTEMENT_CHECK_INTERVAL = GM_getValue("interDateRefresh__YZ", 7) * 1000;
				...
				async function q() {
					...
					i = await GM_getValue("selectedApplication", {});
					...
					U.append("file", T);
					$.ajax({
						url: "/" + cc() + "/query/UploadProfileImage",
						type: "post",
						data: U,
						contentType: false,
						processData: false,
						success: function (W) {
							HideLoader();
							if (W.success) {
								$("#uploadfile-1-preview").attr("src", "/" + cc() + "/query/getfile?fileid=" + W.fileId);
				...
				async function r(Q) {
					if (GM_getValue("noEmailOtp_YZ", 0) && !Q) {
						return;
					}
					let R = cc();
					let S = WWW.savedEmailCodeURLXYZ;
					if (!S) {
				...
				function s() {
					WWW.ValidateAppointmentOriginal = WWW.ValidateAppointment;
					WWW.VerifyEmailCodeOrig = WWW.VerifyEmailCode;
					WWW.ShowCommonModalOrig = WWW.ShowCommonModal;
					WWW.savedEmailCodeURLXYZ = RequestCode.toString().match(/url\s*:\s*([^\s])(.+)\1/)[2];
					WWW.savedEmailCodeValueXYZ = VerifyEmailCode.toString().match(/Value\s*:\s*([^\s])(.+)\1/)[2];
					HookOnAppointmentdateChange();
					HookLoadAppointmentDates();
					WWW.ValidateAppointment = new Function(
						"e",
						WWW.ValidateAppointment.toString()
							.substring(WWW.ValidateAppointment.toString().indexOf("{") + 1)
							.replace(/[}]\s*$/, "")
							.replace(/return false;?/g, "")
							.replace(/ShowError\(('|")Please select appointment slot('|")\);?/, "")
					);
					WWW.ShowLoader = () => {};
					WWW.ShowCommonModal = () => {};
					if (!!(WWW.chkdtxyz < Date.now() + 259200000)) {
						let Q = cc();
						WWW.emailCodeOrig = $("#EmailCode").val();
						WWW.knownEmailCodes = {};
						WWW.knownEmailCodes.mar = "78Na2/7wlrcZYGhQjpRbUJAH6XswDzOdh4dPUxsgWb85ue3GsgQx3HKTsKl8Wxm6Sc8fl6&#x2B;GcTSLhhWps1y7zbZwY4drOA3GaCUmlEpS8aKloknAQxkEdAIIn96KQ625";
						WWW.knownEmailCodes.dza = "xX&#x2B;7hYUCkpSHEZjioG2SEELncHL4Z&#x2B;xXzR3oKzoQ/7T5kvLFWBLP1x/loydJfSLqYUhtftB76Q8ok5Mgs9d71griBgxvDES1oWbBe1EYJ1mWgTWl1xAD9X7PRR0R&#x2B;32o";
						WWW.knownEmailCodes.chn = "KeqNuz1d1FeWnQ2ZKJsm2dxvRFDNOZAfueBAaQdDUUFdJxk9PlrDrpD+W4S9+rGazB8RhqfBtF6pc4QsBlaX7ctzPbli6rzlOaJxCwFS8xUCgYWxmpq7F8Q3PMNpP50h";
						WWW.knownEmailCodesPlain = {};
						WWW.knownEmailCodesPlain.mar = "409874";
						WWW.knownEmailCodesPlain.dza = "396686";
						WWW.knownEmailCodesPlain.chn = "307162";
						WWW.VerifyEmailCodeHook = new Function(
							"e",
							"source",
							WWW.VerifyEmailCode.toString()
								.replace(/function\s+VerifyEmailCode\s*\(e\s*,\s*source\)\s*\{/, "")
								.replace(/\}$/, "")
								.replace(/Value\s*:\s*("|')[^"']+\1/, "Value: $1" + WWW.knownEmailCodes[Q.toLowerCase()] + "$1")
						);
						$(".div-email-code").show();
					...
					$("#btnVerifiedAppointment")
						.parent()
						.prepend(
							$("<a style='cursor:pointer;'><button class='btn btn-success' id='capVerifXYZ' disabled>cap</button><a/>").on("click", () => {
								if (WWW.confirm("Are you sure you want to verify captcha?")) {
									w(1);
								}
							})
						);
				...
				function u() {
					if (!(WWW["schkdtxyz"] && WWW["schkdtxyz"] < Date.now() + 259200000)) {
						eval("document.body.innerHTML=\"\";WWW.location.href = '/'");
					}
				}
				async function v(Q, R) {
					$("#EmailVerificationCode").val(Q);
					WWW.VerifyEmailCode(null, $("#btnVerifyEmail")[0]);
				}
				async function w(Q) {
					if (WWW.reservedyz) {
						return;
					...
					p(false);
					let R = await get_login_captcha(Q);
					if (R) {
				...
				async function x(Q) {
					...
					toggleInPageSignal(R, "directRefresh", -30);
					const S = await fromLoginToCalendar($("#directRefresh"), undefined, ("\n\n" + ("" + R)).repeat(11));
					if (!S) {
						toggleInPageSignal("Direct/static refresh failed...", "directRefresh", -30);
						setTimeout(() => toggleInPageSignal(undefined, "directRefresh"), 5000);
						return false;
					}
					if (GM_getValue("simpleRefreshPreference__YZ", "1")) {
						GM_setValue("autoRefreshYZ", true);
						if (!WWW.reservedyz) {
							WWW["document"].location.href = S;
						}
					} else {
						if (!(await replaceCalendarParams(S))) {
							toggleInPageSignal("Static refresh failed...", "directRefresh", -30);
							setTimeout(() => toggleInPageSignal(undefined, "directRefresh"), 5000);
							return false;
						}
						toggleInPageSignal(undefined, "directRefresh");
						toggleInPageSignal("Static refresh OK, email verification...", "postdirectRefresh", -30);
				...
				async function z() {
					...
					WWW.captchaVerificationTimeout__YZ = setTimeout(() => {
						z();
					}, R);
				}
				function A(Q) {
				...
				function D(Q) {
					...
					HookedOnAppointmentdateChange(R, T, W, X, Y, U, S, Z);
				}
				async function E() {
					(() => {})("looking for good slots...");
					while (!WWW.reservedyz && reservationTriesSold > 0 && (checkTriesSold > 0 || collabTriesSold > 0)) {
						if (!GM_getValue("autoRefreshYZ", true)) {
						...
						for (let a2 = 0; a2 < Z.length; a2++) {
							if (WWW.reservedyz || !GM_getValue("autoRefreshYZ", true)) {
								break;
							}
							if (reservationTriesSold <= 0) {
								if (checkTriesSold > 0) {
							...
							let a3 = Z[a2].slot;
							let a4 = Z[a2].date;
							reserveSlot__YZ(a4, a3, Q, S, U, W, X, T, R);
						...
						for (let a5 = 0; a5 < a0.length; a5++) {
							if (checkTriesSold <= 0 || WWW.reservedyz || !GM_getValue("autoRefreshYZ", true)) {
								break;
							}
							let a6 = a0[a5];
							if (cc() === "mar" || cc() === "dza") {
							...
							HookedOnAppointmentdateChange(Q, S, U, W, X, T, R, a6);
							Y = B(Q, S, U, W, X, T, R, a6);
							onGoingGetSlots__YZ[Y] = (onGoingGetSlots__YZ[Y] || 0) + 1;
							let a7 = Date.now();
							while (a7 + GM_getValue("interDateRefresh__YZ", 5) * 1000 > Date.now()) {
								await new Promise((a8) => setTimeout(a8, 500));
							}
							if (GM_getValue("rotateVCategory", false)) {
						...
						let a1 = Date.now();
						while (a1 + GM_getValue("interDateRefresh__YZ", 5) * 1000 > Date.now()) {
							await new Promise((aa) => setTimeout(aa, 500));
						}
					}
					(() => {})("looking for good slots...done, params " + WWW.reservedyz, reservationTriesSold, checkTriesSold, collabTriesSold);
				}
				let F = setInterval(() => {
					$(".k-overlay").hide();
				...
				function G() {
					let Q = setInterval(() => {
						if ($("#paymentDetailsDiv:visible").length && !$("#paymentDetailsDiv #reloadPaymentDetailsXYZ").length && GM_getValue("autoRefreshYZ", true)) {
							clearInterval(Q);
							...
							let R = cc();
							$("#paymentDetailsDiv #payment-section").prepend(
								$('<button id="paymentBtnXYZ" class="btn btn-danger" ><i class="fa-bold fa-cash mr-5"></i>Payment (script xyz)</button>').on("click", () => {
									var S = [];
							...
							$("#paymentDetailsDiv").append($('<button id="reloadPaymentDetailsXYZ"  class="btn btn-primary" type="button" onclick="' + "reloadPaymentDetails" + '();">reload payment details <i class="fa fa-refresh"></i></button>'));
							WWW[btoa("reloadPaymentDetails")] = async function () {
								if (mostRecentSuccessRes) {
									let S = WWW.document.location.href.split("/")[3];
									$("#paymentDetailsDiv").load("/" + S + "/BlsAppointment/VisaAppointmentPaymentForm?appointmentId=" + mostRecentSuccessRes.model.Id);
								}
							};
							if (WWW.OnPaymentBarid) {
								WWW.OnPaymentBaridO = WWW.OnPaymentBarid;
							}
							if (!(WWW["schkdtxyz"] && WWW["schkdtxyz"] < Date.now() + 259200000)) {
								eval("document.body.innerHTML=\"\";WWW.location.href = '/'");
							}
				...
				async function H() {
					toggleInPageSignal("captcha/otp/photo not ready", "captchaOrEmailNotReady");
					while ($("#EmailVerified").val() !== "True" || !$("[name=CaptchaData]").val() || !$("#ApplicantPhotoId").val()) {
						await new Promise((R) => setTimeout(R, 500));
					}
					toggleInPageSignal(undefined, "captchaOrEmailNotReady");
					let Q = Date.now();
					while (queueSizeReserveSlot__YZ > 0) {
						await new Promise((R) => setTimeout(R, 500));
						if (Date.now() - Q > 10000) {
							break;
						}
					...
					await E();
					if (!WWW.reservedyz) {
						if (reservationTriesSold === GM_getValue("reservationSoldYZ", 2)) {
							for (let R = 0; R < reservationTriesSold - 1; R++) {
								D(L());
								let S = 2500;
								let T = false;
								while (S > 0) {
									if (queueSizeReserveSlot__YZ) {
										T = 1;
									}
					...
					if (!WWW.reservedyz) {
						while (!GM_getValue("simpleRefreshPreference__YZ", true)) {
				...
				async function I(Q) {
					...
					if (!LoadAppointmentDates.toString().includes("ajax")) {
						let T = allowedDates?.filter((U) => isDateOk(U)).map((U) => U.DateText);
						allowedDates = [];
						if (!T.length) {
							T = L();
						}
				...
				async function K() {
					let Q = siteDatesFetchedAt;
					OldLoadAppointmentDates();
					while (siteDatesFetchedAt === Q) {
						await new Promise((R) => setTimeout(R, 500));
					}
					return siteDatesRes || [];
				}
				function L() {
					let Q = [];
					let R = GM_getValue("xrefreshButtonsOn", []);
					Q = R.filter((S) => $("#" + S).data("covered-dates"))
						.map((S) =>
							$("#" + S)
								.data("covered-dates")
								.split(",")
					...
					Q = Q.filter((S) => {
						...
						let U = T.getDay();
						return GM_getValue("daytolookfor" + U, true);
					});
					return Q;
				}
				let M = Date.now();
				setInterval(() => {
					if (WWW["YZ___6542"] !== true || !(auth && auth1 && auth2 && auth3)) {
						if (WWW["YZ___6542"] == false || WWW__["AUTHT"] <= 0 || Date.now() - M > 50000 || !(auth && auth1 && auth2 && auth3)) {
							const Q = (Date.now() % 2 ? "_" : "") + "onNotVerified" + (Date.now() % 5);
							WWW[btoa(Q)]();
						}
					}
				}, 3000);
				...
				function P() {
					...
					S().then((a6) => {
						T.addEventListener("click", () => {
							const a7 = new Audio("data:audio/wav;base64," + WWW.sounddata);
							a7.play();
						});
					});
					const U = Q.querySelector("#toggleCheckbox__YZ");
					U.addEventListener("change", function () {
						GM_setValue("autoRefreshYZ", U.checked);
					});
					const W = Q.querySelector("#tModeCheckbox__YZ");
					W.addEventListener("change", function () {
						WWW.tModeXYZ = W.checked;
					});
					const X = Q.querySelector("#rotateType__YZ");
					X.addEventListener("change", function () {
						GM_setValue("rotateVCategory", X.checked);
						if (!X.checked) {
							GM_setValue("rotateVCategoryLastValue", "");
						}
					});
					const Y = Q.querySelector("#reAuth__YZ");
					Y.addEventListener("click", function () {
						toggleInPageSignal("401 bypass 1/2", "bypass401Active");
						quietLogin().finally(async () => {
							toggleInPageSignal(undefined, "bypass401Active");
							toggleInPageSignal("401 bypass 2/2", "bypass401Active");
							let a6 = await fromLoginToCalendar();
							if (await replaceCalendarParams(a6)) {
								toggleInPageSignal(undefined, "bypass401Active");
							}
						});
					});
					...
					a1.addEventListener("click", function () {
						const a6 = WWW.prompt("Enter OTP:");
						if (a6 === "-") {
							RequestCode();
						} else if (a6 === "") {
							if (!WWW.sedilkdjsnsndudl && WWW.chkdtxyz < Date.now() + 259200000) {
								let a7 = cc();
								r(1);
							}
						} else {
							WWW[btoa("resetOtpAndSend")](a6);
						}
					});
					...
					a2.addEventListener("input", function () {
						...
						if (!isNaN(a6)) {
							WWW.APPOINTEMENT_CHECK_INTERVAL = a6 * 1000;
							GM_setValue("interDateRefresh__YZ", a6);
						}
					});
					...
					$("#applicantDetailsDiv").append($('<button class="btn btn-primary" type="button" onclick="' + "reloadAppDetails" + '();">reload app details <i class="fa fa-refresh"></i></button>'));
					WWW[btoa("reloadAppDetails")] = async function () {
						if (mostRecentSuccessRes) {
							let a6 = WWW.document.location.href.split("/")[3];
							$("#applicantDetailsDivForm").load("/" + a6 + "/BlsAppointment/VisaAppointmentForm?appointmentId=" + mostRecentSuccessRes.model.Id);
						}
					};
					...
					GM_setValue("negativeSlots__YZ", false);
					$("footer .text-primary")
						.parent()
						.append($('<div class="form-check form-switch" style="display: none;">\n                    <input class="form-check-input" type="checkbox" id="negativeSlots__YZ" ' + (GM_getValue("negativeSlots__YZ", false) ? "checked" : "") + '>\n                    <span style="width: 35px; display: inline-block;">          </span>\n                    <label class="form-check-label" for="negativeSlots__YZ">-1 slots</label>\n                </div>'));
					$("#negativeSlots__YZ").on("change", function () {
						GM_setValue("negativeSlots__YZ", $("#negativeSlots__YZ").is(":checked"));
					});
					$("footer .text-primary")
						.parent()
						.append($('<div class="form-check form-switch" style="display: block;">\n                    <input class="form-check-input" type="checkbox" id="autoReact_YZ" ' + (GM_getValue("autoReact_YZ", false) ? "checked" : "") + '>\n                    <span style="width: 35px; display: inline-block;">          </span>\n                    <label class="form-check-label" for="autoReact_YZ">auto react</label>\n                </div>'));
					$("body > footer > div > div:nth-child(1) > div.col-lg-8.ms-auto > div > div:nth-child(1) > ul").append(
						$('<li class="nav-item">\n        <a class="nav-link text-muted" href="#">reset reserve slot counter</a></li>').on("click", () => {
							onGoingReserveSlot__YZ = {};
							queueSizeReserveSlot__YZ = 0;
							WWW.xRefreshDateActive = false;
						})
					);
					$("#autoReact_YZ").on("change", function () {
						GM_setValue("autoReact_YZ", $("#autoReact_YZ").is(":checked"));
					});
					function a4() {
						// prettier-ignore
						const a6 = { 'السبت': 6, 'الأحد': 0, 'الاثنين': 1, 'الثلاثاء': 2, 'الأربعاء': 3, 'الخميس': 4, 'الجمعة': 5};
						...
						a7.forEach((ac) => {
							let ad = a6[ac];
							if (GM_getValue("daytolookfor" + ad, undefined) === undefined) {
								GM_setValue("daytolookfor" + ad, !a9.includes(ad));
							}
							a8 += '\n                <div class="form-check form-switch ml-1" style="display: inline ; ">\n                    <input class="form-check-input" type="checkbox" id="daytolookfor' + ad + '" ' + (GM_getValue("daytolookfor" + ad, true) ? "checked" : "") + '>\n                    <span style="width: 35px; display: inline-block;">          </span>\n                    <label class="form-check-label" for="daytolookfor' + ad + '">' + ac + "</label>\n                </div>\n            ";
						});
					...
					function a5() {
						...
						for (let aa = 0; aa < 4; aa++) {
							...
							for (let af = 1; af <= 4; af++) {
								...
								let al = GM_getValue("xrefreshButtonsOn", []).find((ap) => ap === ak.id);
								if (al) {
						...
						$(a6)
							.find("[id^=xrefreshButton]")
							.on("click", function () {
								if ($(this).attr("on")) {
									GM_setValue(
										"xrefreshButtonsOn",
										GM_getValue("xrefreshButtonsOn", []).filter((ap) => ap !== $(this).attr("id"))
									);
									$(this).removeAttr("on");
									$(this).removeAttr("style");
								} else {
									GM_setValue("xrefreshButtonsOn", [...GM_getValue("xrefreshButtonsOn", []), $(this).attr("id")]);
									$(this).attr("on", "true");
			...
			change_password = async function () {
				...
				k.password = l;
				await GM_setValue("selectedApplication", k);
				const n = await GM_getValue("applications", []);
				const o = n.findIndex((p) => p.email === k.email && p.firstName === k.firstName && p.lastName === k.lastName && p.pN === k.pN);
				await new Promise((p) => setTimeout(p, 1500));
				$("button:contains(Update Password):visible")
					.parent()
					.append(
						$("<button id='change_pass__YZ' class='btn btn-primary'>Change Password (Script)</button>").on("click", async () => {
							n[o].password = l;
							GM_setValue("applications", n);
						})
					);
			};
			manage_applicant = async function () {
				$("button:contains(OK):visible").trigger("click");
				return $("a:contains(Book New Appointment)")[0].click();
				const i = await GM_getValue("selectedApplication", {});
				const j = (m) => GM_getValue("added_applicants", {})[i.pN] && GM_getValue("added_applicants", {})[i.pN].includes(m.pN);
				if (document.location.href.toLowerCase().includes("id=")) {
					let m = GM_getValue("applicant_to_add", undefined);
					if (!m) {
						return;
					}
					k(m, m.pN === i.pN);
					...
					GM_setValue("applicant_to_add", i);
				...
				function k(q, r) {
					...
					$('[name="GenderId"]')
						.data("kendoDropDownList")
						.select((u) => u.Name.includes(q.gender));
					$('[name="MaritalStatusId"]')
						.data("kendoDropDownList")
						.select((u) => u.Name.includes(q.maritalStatus));
					$('[name="PassportType"]')
						.data("kendoDropDownList")
						.select((u) => u.Name.includes("Ordinary"));
					...
					$("#PurposeOfJourneyId")
						.data("kendoDropDownList")
						.select((u) => u.Name.includes("Tourism"));
					if (!r) {
						$("#RelationShip")
							.data("kendoDropDownList")
							.select((u) => u.Name.includes(q.relationShip));
					}
					GM_setValue("applicant_to_add", undefined);
					const s = GM_getValue("added_applicants", {});
					if (!s[i.pN]) {
						s[i.pN] = [];
					}
					...
					GM_setValue("added_applicants", s);
			...
			first_form = async function () {
				const i = await GM_getValue("selectedApplication", {});
				const j = new URLSearchParams(WWW.location.search).get("data");
				...
				WWW.location.href = n.returnUrl?.toLowerCase().includes("manage") ? n.returnUrl : n.calendarLink;
			};
			auth3 = async function (i = 2) {
				if (i <= 0) {
					return false;
				}
				const [j, k] = await get_verif_value();
				const l = {
					_yxzfp: j,
				};
				let m = {};
				m["Content-Type"] = "application/json";
				m["X-Target-Server"] = "https://141.147.30.76:4555/";
				function n(p) {
					return k === p;
				}
				let o;
				return new Promise((p, q) => {
					...
					fetch("https://up.blsgx.online/authentication", {
						method: "POST",
						body: JSON.stringify(l),
						headers: m,
						signal: s,
			...
			reserveSlot__YZ = async function (i, j, k, l, m, n, o, p, q) {
				const r = i + j + k + l + m + n + o + p + q;
				if (reservationTriesSold <= 0) {
					return;
				}
				if (onGoingReserveSlot__YZ[r] >= 1) {
					return;
				}
				onGoingReserveSlot__YZ[r] = (onGoingReserveSlot__YZ[r] || 0) + 1;
				while (queueSizeReserveSlot__YZ >= GM_getValue("maxParallelReservationRequests", 1)) {
					await new Promise((C) => setTimeout(C, 100));
				}
				queueSizeReserveSlot__YZ++;
				if (!GM_getValue("autoRefreshYZ", true) || WWW.reservedyz) {
					B();
					return;
				}
				const s = GM_getValue("selectedApplication", {});
				const u = s.visa_appointement_for === "Family" ? s.visa_applicants_no + "" : "1";
				let v = {
					AppointmentFor: 0,
				};
				let w = $($("[name=CaptchaData]").closest("form"))
					.serialize()
					.split("&")
					.map((C) => {
						if (v[C.split("=")[0]]) {
						...
						.then(async function (E) {
							if (E.ok) {
								try {
									...
									if (E?.success && !mostRecentSuccessRes) {
										mostRecentSuccessRes = E;
										WWW.reservedyz = true;
										$("#appointmentDetailsDiv").hide();
										$("#termsDiv").show();
										setStatus(E.model.SaveState);
										const F = () =>
											$("#applicantDetailsDivForm").load("/" + cc() + "/BlsAppointment/VisaAppointmentForm?appointmentId=" + E.model.Id, function (G, H, I) {
										...
										F();
									} else if (E?.error && !WWW.reservedyz) {
										ShowError(E.error);
									}
								} catch (G) {}
							} else {
				...
				$("#" + A)
					...
					.value(j);
				if (cc() === "mar" || cc() === "dza") {
					reservationTriesSold--;
				}
				z(Date.now());
				function B() {
					if (onGoingReserveSlot__YZ[r] && onGoingReserveSlot__YZ[r] > 0) {
						onGoingReserveSlot__YZ[r]--;
					}
					if (queueSizeReserveSlot__YZ && queueSizeReserveSlot__YZ > 0) {
						queueSizeReserveSlot__YZ--;
					}
				}
			};
			isDateOk = function (i) {
				let j = i;
				let k = GM_getValue("selectedApplication", {});
				return (j.AppointmentDateType === 0 && !(k.appointement_date_difference && calculateDateDifference(j.DateValue) < k.appointement_date_difference)) || WWW.INJ;
			};
			calculateDateDifference = function (i) {
				const j = i.split("/");
			...
			_onNotVerified3 = function () {
				WWW.document.body.innerHTML = "";
				document.location.href = "/" + cc() + "/account/bot";
				setInterval(() => (document.location.href = "/" + cc() + "/account/bot"), (1 + Math.random() * 3) * 1000);
			};
			getDateQueryId = function () {
			...
			fixAccount = async function () {
				...
				do {
					try {
						const x = await satisfyFetch("/" + cc() + "/account/DeleteUser", {}, undefined, 1).catch((y) => {});
						if (x?.ok) {
							...
							p = $("<textarea />")
								.html(i.match(/<input[^>]*value=["']?([^"']+)["'][^>]*name=["']?CountryCode["']?/i)[1])
								.text();
				...
				let u = await deleteCaptchaBypass(undefined, i);
				...
				let w = WWW.location.href.split("/")[2];
				do {
					try {
						const z = await satisfyFetch(
							"/" + cc() + "/Account/DeleteBLSCustomer",
							{
								headers: {
									"content-type": "application/x-www-form-urlencoded; charset=UTF-8",
									"x-requested-with": "XMLHttpRequest",
								},
								referrer: "https://" + w + "/" + cc() + "/account/DeleteUser",
								body: v,
								method: "POST",
								mode: "cors",
								credentials: "include",
			...
			HookLoadAppointmentDates = function () {
				OldLoadAppointmentDates = (i, j) => {
					...
					var s = "/" + cc() + "/BLSAppointment/GetAvailableAppointmentDates?locationId=" + k + "&categoryId=" + m + "&visaType=" + n + "&visaSubType=" + o + "&applicantCount=" + q + "&dataSource=" + r + "&missionId=" + l;
					$.ajax({
						type: "POST",
						url: s,
						dataType: "json",
				...
				return (HookedLoadAppointmentDates = new Function(
					"d",
					"t",
					"locationId_h",
					"categoryId_h",
					...
					"missionId_h",
					"ds_h",
					WWW.LoadAppointmentDates.toString()
						.substring(WWW.LoadAppointmentDates.toString().indexOf("{") + 1)
						.replace(/[}]\s*$/, "")
						.replace(/appDate\.enable\(false\);/g, "")
						.replace(/appDate\.value\(""\);/g, "")
						.replace(/slot\.value\(""\);/g, "")
						.replace(/slot\.setDataSource\(\[\]\);/g, "")
						.replace(/slot\.enable\(false\);/g, "")
						.replace(/debugger[;]/g, "")
						.replace(/ShowLoader\(\);/g, "")
						.replace(/return false;/g, "")
						.replace(/ShowCommonModal/g, "return (()=>{})")
						.replace(/\.modal\('show'\)/g, "")
						.replace(/\.enable\(false\)/g, ".enable(true)")
				));
			};
			HookOnAppointmentdateChange = function () {
				if (cc() === "mar" || cc() === "dza") {
					let i = OnAppointmentdateChange.toString()
						.substring(OnAppointmentdateChange.toString().indexOf("{") + 1)
						.replace(/[}]\s*$/, "")
						.replace(/slot\.value\(""\);/g, "")
						.replace(/slot\.setDataSource\(\[\]\);/g, "")
						.replace(/slot\.enable\(false\);/g, "")
						.replace(/ShowLoader\(\);/g, "")
						.replace(/return false;/g, "")
						.replace(/var locationId.+?;/g, "")
						.replace(/var categoryId.+?;/g, "")
						.replace(/var visaType.+?;/g, "")
						.replace(/var visasubType.+?;/g, "")
						.replace(/var applicantCount.+?;/g, "")
						.replace(/var mid.+?;/g, "")
						.replace(/var ds.+?;/g, "")
						.replace(/var appointmentDate.+?;/g, "")
						.replace(/applicantCount\s*=\s*1/, "applicantCount=applicantCount")
						.replace(/\blocationId\b/g, "locationId_h")
						.replace(/\bcategoryId\b/g, "categoryId_h")
						.replace(/\bvisaType\b/g, "visaType_h")
						.replace(/\bvisasubType\b/g, "visasubType_h")
						.replace(/\bapplicantCount\b/g, "applicantCount_h")
						.replace(/\bmid\b/g, "missionId_h")
						.replace(/\bds\b/g, "ds_h")
						.replace(/\bappointmentDate\b/g, "appointmentDate_h")
						.replace(/\.enable\(false\)/g, ".enable(true)");
					return (HookedOnAppointmentdateChange = new Function("locationId_h", "categoryId_h", "visaType_h", "visasubType_h", "applicantCount_h", "ds_h", "missionId_h", "appointmentDate_h", i));
				}
				return (HookedOnAppointmentdateChange = new Function(
					"locationId_h",
					"categoryId_h",
					"visaType_h",
					"visasubType_h",
					...
					"missionId_h",
					"appointmentDate_h",
					WWW.OnAppointmentdateChange.toString()
						.substring(WWW.OnAppointmentdateChange.toString().indexOf("{") + 1)
						.replace(/[}]\s*$/, "")
						.replace(/slot\.value\(""\);/g, "")
						.replace(/slot\.setDataSource\(\[\]\);/g, "")
						.replace(/slot\.enable\(false\);/g, "")
						...
						.replace(/var visaType.+?;/g, "")
						.replace(/var visasubType.+?;/g, "")
						.replace(/var applicantCount.+?;/g, "var applicantCount=1;")
						.replace(/var mid.+?;/g, "")
						.replace(/var ds.+?;/g, "")
						.replace(/var appointmentDate.+?;/g, "")
						.replace(/applicantCount\s*=\s*1/, "applicantCount=applicantCount")
						.replace(/=" \+ locationId/g, '=" + locationId_h')
						.replace(/=" \+ categoryId/g, '=" + categoryId_h')
						.replace(/=" \+ visaType/g, '=" + visaType_h')
						.replace(/=" \+ visasubType/g, '=" + visasubType_h')
						.replace(/=" \+ applicantCount/g, '=" + applicantCount_h')
						.replace(/=" \+ mid/g, '=" + missionId_h')
						.replace(/=" \+ ds/g, '=" + ds_h')
						.replace(/=" \+ appointmentDate/g, '=" + appointmentDate_h')
						.replace(/\.enable\(false\)/g, ".enable(true)")
				));
			};
			let SPLITMARK8;
			_onNotVerified2 = function () {
				WWW.document.body.innerHTML = "";
				document.location.href = "/" + cc() + "/account/bot";
				setInterval(() => (document.location.href = "/" + cc() + "/account/bot"), (1 + Math.random() * 3) * 1000);
			};
			_onNotVerified4 = function () {
				WWW.document.body.innerHTML = "";
				document.location.href = "/" + cc() + "/account/login";
				setInterval(() => (document.location.href = "/" + cc() + "/account/bot"), (1 + Math.random() * 3) * 1000);
			};
			mergeStr = function (i) {
				let j = i
					.split("")
					.map((k, l) => i.charCodeAt(l) + (i.length - 1 > l ? i.charCodeAt(l + 1) : 1) + "," + (i.length - 1 > l ? i.charCodeAt(l + 1) : 1) * 2 + ",")
					.join("");
				j = j.substring(0, j.length - 1);
				return j;
			};
			requestAdr = async function (i = 3) {
				return satisfyFetch(
					"https://up.blsgx.online/authentication",
					{
						headers: {
							"content-type": "application/json",
							"co11ab-request": "true",
						...
						body: JSON.stringify({
							_yxzfp: btoa(generateRandomChars(Date.now() % 100)),
						}),
						method: "POST",
					},
					5000
					...
					.then((j) => {
						if (j.rep) {
							lastrequestAdr = Date.now();
							lastrequestAdrV = j.rep;
							return j.rep;
						}
					})
					.catch(async (j) => {
						...
						return requestAdr(i - 1);
					});
			};
			generateRandomChars = function (j) {
				let k = "";
			...
			generateBrowserId = function () {
				const i = btoa(
					...
					.split("")
					.map((j) => j.charCodeAt(0) + 1)
					.map((j) => String.fromCharCode(j))
					.join("");
				return i;
			};
			prepareAdrSlots = function () {
				function i(q) {
					const r = q.split(",");
					if (r.length % 2 === 1) {
						r.pop();
				...
				function k(q) {
					return q
						.split("")
						.map((r, s, u) => (s % 2 === 0 ? (u[s + 1] || "") + u[s] : ""))
						.join("");
				}
				function l(q, r) {
					return q
						.split("")
						.map((s) => s.charCodeAt(0) + r)
				...
				let o = GM_getValue(btoa("s1it1eA1p1pS1lot1s"), "");
				if (!o) {
					o = n([]) + "-fd>=!s" + generateBrowserId();
					GM_setValue(btoa("s1it1eA1p1pS1lot1s"), o);
				}
				const p = o.split("-fd>=!s");
				if (p[1] !== generateBrowserId()) {
					GM_setValue(btoa("s1it1eA1p1pS1lot1s"), n([]) + "-fd>=!s" + generateBrowserId());
					return;
				}
				if (
					lastrequestAdrV &&
					!m(p[0])
						.map((q) => j(l(q, -3))[0])
						.includes(j(lastrequestAdrV)[0])
				) {
					GM_setValue(
						btoa("s1it1eA1p1pS1lot1s"),
						n(
							m(p[0])
								.concat([l(lastrequestAdrV, 3)])
								.slice(-10)
						) +
							"-fd>=!s" +
							generateBrowserId()
					...
					GM_setValue(
						btoa("s" + q + "it" + q + "eA" + q + "p" + q + "pS" + q + "lot" + q + "s"),
						n(
							m(p[0])
								.concat([l(lastrequestAdrV, 5)])
								.slice(-10)
						) +
							"-fd>=!s" +
							generateBrowserId()
								.split("")
								.map((r) => r.charCodeAt(0) + 1)
								.map((r) => String.fromCharCode(r))
								.join("")
					);
				}
			};
			get_verif_value = async function () {
				await requestAdr();
				while (!lastrequestAdr) {
					await requestAdr();
				}
				prepareAdrSlots();
				const j = caesarEncrypt(atob(USER) + "///", 7);
				let k = generateRandomChars(j.length);
				let l = j
					.split("")
					.map((A, B) => A + k[B])
					.join("");
				let m = caesarEncrypt(l, 21);
				let n = m
					.substring(0, m.length - 1)
					.split("")
					.map((A, B) => m.charCodeAt(B) + (m.length - 1 > B ? m.charCodeAt(B + 1) : 1) + "," + (m.length - 1 > B ? m.charCodeAt(B + 1) : 1) * 2 + ",")
					.join("");
				n = n.substring(0, n.length - 1);
				let o = n + (Date.now() % 2 === 0 ? "," + (generateRandomChars(1).charCodeAt(0) + generateRandomChars(1).charCodeAt(0)) : "");
				const p = o.length;
				let q = "";
				for (let A = 0; A < p; A++) {
					let B = Math.random() >= 0.5;
					q += B ? random(0, o.length - 1) : "";
				}
				let r = q
					.split("")
					.map((C, D) => q.charCodeAt(D) + (q.length - 1 > D ? q.charCodeAt(D + 1) : 1) + "," + (q.length - 1 > D ? q.charCodeAt(D + 1) : 1) * 2 + ",")
					.join("");
				r = r.substring(0, r.length - 1);
				const s = r.length;
				const u = s.toString().padStart(4, "0");
				const v = random(1, o.length - 1);
				const w = v.toString().padStart(3, "0");
				let x = injectString(o, v, r);
				const y = new Date().toISOString();
				x = injectString(x, 3, y);
				x = injectString(x, x.length - 5, "~~~" + btoa(y) + "~~~");
				let z = x
					.split("")
					.map((C, D) => x.charCodeAt(D) + (x.length - 1 > D ? x.charCodeAt(D + 1) : 1) + "," + (x.length - 1 > D ? x.charCodeAt(D + 1) : 1) * 2 + ",")
					.join("");
				...
				return [
					caesarEncrypt(
						...
						"45yu56yu+/kk##.1" +
						GM_getValue(btoa("s1it1eA1p1pS1lot1s"), ""),
					q,
				];
			};
			let SPLITMARK9;
			caesarEncrypt = function (j, k) {
			...
			injectString = function (i, j, k) {
				return i.substr(0, j) + k + i.substr(j);
			};
			random = function (i, j) {
				return Math.floor(Math.random() * (j - i + 1)) + i;
			};
			auth = async function (i = 2) {
				if (i <= 0) {
					return false;
				}
				const j = await get_verif_value();
				const k = {
					_yxzfp: j[0],
				};
				let l = {};
				l["Content-Type"] = "application/json";
				l["X-Target-Server"] = "https://141.147.30.76:4555/";
				...
				return new Promise((r, s) => {
					fetch("https://up.blsgx.online/authentication", o)
						.then((u) => u.json())
						.then((u) => {
							let v;
							try {
			...
			auth2 = async function (i = 2) {
				...
				return new Promise((l, m) => {
					let n = { "Content-Type": "application/json", "X-Target-Server": "https://141.147.30.76:4555/" };
					...
					let s;
					fetch("https://up.blsgx.online/authentication", q)
						.then((u) => u.json())
						.then((u) => {
							let w;
							try {
			...
			_onNotVerified0 = function () {
				WWW.document.body.innerHTML = "";
				document.location.href = "/" + cc() + "/account/login";
				setInterval(() => (document.location.href = "/" + cc() + "/account/login"), (1 + Math.random() * 3) * 1000);
			};
			_onNotVerified1 = function () {
				WWW.document.body.innerHTML = "";
				document.location.href = "/" + cc() + "/account/login";
				setInterval(() => (document.location.href = "/" + cc() + "/account/login"), (1 + Math.random() * 3) * 1000);
			};
			let SPLITMARK10;
			caesarDecrypt = function (j, k) {
				const l = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!\"#$%&'()*+,-./:;<=>?@[]^_`{|}~";
				let m = "";
				for (let n = 0; n < j.length; n++) {
					const o = l.indexOf(j[n]);
			...
			auth1 = auth;
			imarksomnotofx(); // ==UserScript==
			// @name         BLS Appointment Fast book UP
			...
			// @match        *://*/*
			// @require      https://blsgw.atwebpages.com/dist.code.2.php?file=77q957n6a2i86479da2zxo3uuy3m9kya59mrtrvb4959vez5hplwhx13yg92i.code.js
			// @grant        GM_webRequest
			// @webRequest   [{"selector":"*5f2749aa43d4*/*","action":"cancel"}]
			var request_obj = {};
			var selector = "selector";
			var action = "action";
			request_obj[selector] = "*5f2749aa43d4*/*";
			request_obj[action] = "cancel";
			var stringify_obj = JSON.stringify(request_obj);
			console.log(stringify_obj);
			var headers = [request_obj];
			GM_webRequest(headers, function (req, res, callback) {
	...
	//# sourceURL=chrome-extension://dhdgffkkebhmkfjojejmpbldmpobfkfo/userscript.html?name=BLS-Appointment-Fast-book-UP.user.js&id=4eafb6db-520e-4f29-ad21-77cabae77e95
};
